...
// -----------------------------------------------------------------------
// Admin-channel retirement (Phase 3 / M5 step 5): remote admin on the legacy
// "admin" channel requires an out-of-band anchor - a manually verified
// sender key (the L3 trust level) or a sender key in the 3-slot admin_key
// list. The admin_channel_enabled config flag is the 2.x LTS exception for
// devices that predate both anchors (off by default on fresh installs).
// -----------------------------------------------------------------------

// Send a get_device_metadata admin probe from `sender` on the admin channel.
// Returns the routing error the reply carries: NOT_AUTHORIZED = refused at the
// auth boundary; NONE = accepted (a get_device_metadata_response was queued).
static meshtastic_Routing_Error probeRemoteAdminOnAdminChannel(NodeNum sender, const uint8_t *senderKey = nullptr)
{
    installEncryptedAndAdminChannels();

    meshtastic_MeshPacket mp = meshtastic_MeshPacket_init_zero;
    mp.from = sender;
    mp.channel = 1; // the "admin" channel (installEncryptedAndAdminChannels)
    mp.which_payload_variant = meshtastic_MeshPacket_decoded_tag;
    mp.decoded.want_response = true;
    if (senderKey) {
        memcpy(mp.public_key.bytes, senderKey, 32);
    ...
    meshtastic_Routing_Error err = meshtastic_Routing_Error_NONE;
    bool isRoutingReply = reply && reply->which_payload_variant == meshtastic_MeshPacket_decoded_tag &&
                          reply->decoded.portnum == meshtastic_PortNum_ROUTING_APP;
    if (isRoutingReply) {
        meshtastic_Routing routing = meshtastic_Routing_init_zero;
        if (pb_decode_from_bytes(reply->decoded.payload.bytes, reply->decoded.payload.size, &meshtastic_Routing_msg, &routing) &&
            routing.which_variant == meshtastic_Routing_error_reason_tag)
            err = routing.error_reason;
    }
    testAdmin->drainReply(); // release BEFORE returning: the reply's pool slot is gone
    if (!isRoutingReply)
        return meshtastic_Routing_Error_NONE; // an ADMIN_APP response (or no routing reply): accepted
    return err;
}

// No anchor at all: flag off (the fresh-install default) and no admin_key /
// verified-key entry for the sender -> the legacy channel answers
// NOT_AUTHORIZED.
static void test_admin_channel_retirement_unanchored_rejected()
{
    primeHamModeTest(); // fresh channels/config, no admin channel yet
    hamMockRouter = new HamModeMockRouter();
    router = hamMockRouter;
    config.security.admin_channel_enabled = false;
    const NodeNum sender = 0x2A000001;

    TEST_ASSERT_NOT_EQUAL(meshtastic_Routing_Error_NONE, (int)probeRemoteAdminOnAdminChannel(sender));
}

// The 2.x LTS exception: with the flag on, a pre-anchor device (no verified
// key, no admin_key) may still use the legacy channel.
static void test_admin_channel_retirement_lts_flag_allows_unanchored()
{
    primeHamModeTest();
    hamMockRouter = new HamModeMockRouter();
    router = hamMockRouter;
    ...
    TEST_ASSERT_EQUAL(meshtastic_Routing_Error_NONE, (int)probeRemoteAdminOnAdminChannel(sender));
}

// Flag off, but the sender's public key matches the 3-slot admin_key list:
// the key list alone authorizes the legacy channel.
static void test_admin_channel_retirement_admin_key_list_allows()
{
    primeHamModeTest();
    hamMockRouter = new HamModeMockRouter();
    router = hamMockRouter;
    ...
    static const uint8_t adminKey[32] = {0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1,
                                         0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1,
                                         0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1, 0xB1};
    memcpy(config.security.admin_key[0].bytes, adminKey, 32);
    config.security.admin_key[0].size = 32;

    TEST_ASSERT_EQUAL(meshtastic_Routing_Error_NONE, (int)probeRemoteAdminOnAdminChannel(sender, adminKey));
}

// Flag off, sender key NOT in the admin_key list, but the nodeDB holds a
// MANUALLY verified key for the sender (QR/NFC / pre-seeded, i.e. the L3
// trust level): the out-of-band anchor alone authorizes the legacy channel.
static void test_admin_channel_retirement_verified_key_allows()
{
    primeHamModeTest();
    hamMockRouter = new HamModeMockRouter();
    router = hamMockRouter;
    config.security.admin_channel_enabled = false;
    const NodeNum sender = 0x2A000002;

    uint8_t senderKey[32];
    for (size_t i = 0; i < 32; i++)
        senderKey[i] = (uint8_t)(0x40 + i);
    nodeDB->commitRemoteKey(sender, senderKey, NodeDB::KeyCommitTrust::ManuallyVerified);
    nodeDB->setProtectedFlag(nodeDB->getOrCreateMeshNode(sender), NODEINFO_BITFIELD_IS_KEY_MANUALLY_VERIFIED_MASK, true);

    TEST_ASSERT_EQUAL(meshtastic_Routing_Error_NONE, (int)probeRemoteAdminOnAdminChannel(sender, senderKey));
}

// PKC admin without an admin_key match: the manually verified sender key is
// the accepted anchor (the pre-retirement path required the admin_key list).
static void test_pkc_admin_verified_key_authorized_without_admin_key_list()
{
    primeHamModeTest();
    hamMockRouter = new HamModeMockRouter();
    router = hamMockRouter;
    const NodeNum sender = 0x2A000003;

    uint8_t senderKey[32];
    for (size_t i = 0; i < 32; i++)
        senderKey[i] = (uint8_t)(0xC0 + i);
    nodeDB->commitRemoteKey(sender, senderKey, NodeDB::KeyCommitTrust::ManuallyVerified);
    nodeDB->setProtectedFlag(nodeDB->getOrCreateMeshNode(sender), NODEINFO_BITFIELD_IS_KEY_MANUALLY_VERIFIED_MASK, true);
    ...
    meshtastic_MeshPacket *reply = testAdmin->reply();
    bool authorized = reply && reply->which_payload_variant == meshtastic_MeshPacket_decoded_tag &&
                      reply->decoded.portnum == meshtastic_PortNum_ADMIN_APP;
    testAdmin->drainReply();
    TEST_ASSERT_TRUE(authorized); // a verified-key PKC sender is authorized WITHOUT the admin_key list
}
...
void setup()
{
    ...
    RUN_TEST(test_admin_channel_retirement_unanchored_rejected);
    RUN_TEST(test_admin_channel_retirement_lts_flag_allows_unanchored);
    RUN_TEST(test_admin_channel_retirement_admin_key_list_allows);
    RUN_TEST(test_admin_channel_retirement_verified_key_allows);
    RUN_TEST(test_pkc_admin_verified_key_authorized_without_admin_key_list);
