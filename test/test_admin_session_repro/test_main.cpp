...
// Telemetry requests and replies need normal shared-channel encryption so every node with the
// channel key can decode the LocalStats payload. They must not be auto-upgraded to PKC merely
// because both endpoints have identity keys.
void test_telemetry_uses_channel_encryption_not_pkc(void)
{
...
void setup()
{
    ...
    RUN_TEST(test_telemetry_uses_channel_encryption_not_pkc);
