...
public:
  ...
  std::list<std::tuple<meshtastic_Routing_Error, NodeNum, PacketId, ChannelIndex, uint8_t>> ackNacks_; // ackNacks received by the RoutingModule.
};
  ...
  void handleCommand(uint8_t header, std::string_view message) {
    ...
    case MQTTPUBLISH: {
      ...
      if (topic == kTextTopic) {
        ...
        published_.emplace_back(std::move(topic), DecodedServiceEnvelope(reinterpret_cast<const uint8_t *>(message.data()), message.size()));
      }
      break;
    }
    }
...
public:
  MQTTUnitTest() : MQTT(std::make_unique<MockPubSubServer>()) { pubsub = reinterpret_cast<MockPubSubServer *>(mqttClient.get()); }
  ~MQTTUnitTest() {
  ...
  static void restart() {
    ...
    TEST_ASSERT_TRUE(loopUntil([] { return pubsub->subscriptions_.count("msh/2/e/test/+") && pubsub->subscriptions_.count("msh/2/e/PKI/+"); }));
  }
...
void setUp(void) {
  moduleConfig.mqtt = meshtastic_ModuleConfig_MQTTConfig{.enabled = true, .map_reporting_enabled = true, .has_map_report_settings = true};
  moduleConfig.mqtt.map_report_settings =
      meshtastic_ModuleConfig_MapReportSettings{.publish_interval_secs = 0, .position_precision = 14, .should_report_location = true};
  ...
  localPosition = meshtastic_Position{.has_latitude_i = true, .latitude_i = 7 * 1e7, .has_longitude_i = true, .longitude_i = 3 * 1e7};
...
void test_receiveDecodedProtoFromProxy(void) {
  const meshtastic_ServiceEnvelope env = {.packet = const_cast<meshtastic_MeshPacket *>(&decoded), .channel_id = "test", .gateway_id = "!87654321"};
  ...
  message.payload_variant.data.size =
      pb_encode_to_bytes(message.payload_variant.data.bytes, sizeof(message.payload_variant.data.bytes), &meshtastic_ServiceEnvelope_msg, &env);
...
void verifyLatLong(const DecodedServiceEnvelope &env, uint32_t latitude, uint32_t longitude) {
  ...
  TEST_ASSERT_TRUE(pb_decode_from_bytes(p.decoded.payload.bytes, p.decoded.payload.size, &meshtastic_MapReport_msg, &mapReport));
...
void test_usingDefaultServer(void) { TEST_ASSERT_TRUE(mqtt->isUsingDefaultServer()); }
...
void test_enabled(void) { TEST_ASSERT_TRUE(mqtt->isEnabled()); }
...
void test_customMqttRoot(void) {
  ...
  TEST_ASSERT_TRUE(loopUntil([] { return pubsub->subscriptions_.count("custom/2/e/test/+") && pubsub->subscriptions_.count("custom/2/e/PKI/+"); }));
}
