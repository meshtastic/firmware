...
void test_DH25519(void) {
  ...
  crypto->setDHPrivateKey(private_key);
  TEST_ASSERT(!crypto->setDHPublicKey(
      public_key)); // Weak public key results in 0 shared key
...
void test_PKC(void) {
  ...
  HexToBytes(radioBytes, "8c646d7a2909000062d6b2136b00000040df24abfcc30a17a3d90"
                         "46726099e796a1c036a792b");
  HexToBytes(expected_nonce, "62d6b213036a792b2909000000");
  crypto->setDHPrivateKey(private_key);

  TEST_ASSERT(crypto->decryptCurve25519(fromNode, public_key, packetNum, 22,
                                        radioBytes + 16, decrypted));
  ...
  TEST_ASSERT(crypto->encryptCurve25519(toNode, fromNode, public_key, packetNum,
                                        10, decrypted, encrypted));
  TEST_ASSERT_EQUAL_MEMORY(expected_shared, crypto->shared_key, 8);
  // The extraNonce is random, so skip checking the nonce and encrypted output
  // here

  // Copy the nonce to check it after encryption
  memcpy(expected_nonce, crypto->nonce, 16);

  // Decrypt the re-encrypted bytes and check they are the same as what we
  // expect
  TEST_ASSERT(crypto->decryptCurve25519(fromNode, public_key, packetNum, 22,
                                        encrypted, decrypted));
  TEST_ASSERT_EQUAL_MEMORY(expected_shared, crypto->shared_key, 8);
...
// ============================================================================
// PFS (Perfect Forward Secrecy) Tests
// ============================================================================
...
void test_PFS_BackwardCompatibility(void) {
  ...
  // Legacy decryption should still work
  TEST_ASSERT(crypto->decryptCurve25519(fromNode, public_key, packetNum, 22,
                                        radioBytes + 16, decrypted));
  TEST_ASSERT_EQUAL_MEMORY(expected_decrypted, decrypted, 10);

  // Legacy encryption should still work
  uint32_t toNode = 0;
  uint8_t encrypted[128] __attribute__((__aligned__));
  TEST_ASSERT(crypto->encryptCurve25519(toNode, fromNode, public_key, packetNum,
                                        10, decrypted, encrypted));

  // Re-decrypt and verify
  TEST_ASSERT(crypto->decryptCurve25519(fromNode, public_key, packetNum, 22,
                                        encrypted, decrypted));
  TEST_ASSERT_EQUAL_MEMORY(expected_decrypted, decrypted, 10);
}
