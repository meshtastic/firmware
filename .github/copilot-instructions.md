...
- **Persistence:** nRF52840 uses a 12 KB raw-flash record-ring at `0xEA000` (below LittleFS; append + replay + compact-on-rotate, link-guarded by `nrf52840_s140_v7.ld` and `extra_scripts/nrf52_warm_region.py`). On every other arch with `WARM_NODE_COUNT > 0` (nRF54 included, STM32WL has none): a `/prefs/warm.dat` snapshot flushed by `saveIfDirty()` on the node-DB save cadence.
