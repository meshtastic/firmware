...
- **nRF54L** (nRF54L15) - `ARCH_NRF54L`, built on the nRF52 platform layer (`src/platform/nrf52`, `-Isrc/platform/nrf52`) through the out-of-tree `meshtastic/platform-nordicnrf54` platform and its s145 SoftDevice Arduino core; the divergent spots (crypto engine, BLE, DFU, RNG, boot) are `#ifdef ARCH_NRF54L`. Boards under `variants/nrf54l15/`: `xiao_nrf54l15_lr2021` is the per-PR canary, `xiao_nrf54l15` and `nrf54l15dk` are `board_level = extra`. Memory class SMALL: 120 hot nodes, 100 warm in `/prefs/warm.dat` (not the nRF52840 raw-flash ring)
...
On every arch except STM32WL (`WARM_NODE_COUNT > 0`; the only `MEM_CLASS_TINY` part), a node evicted from the header table is not forgotten outright: `WarmNodeStore` (`src/mesh/WarmNodeStore.{h,cpp}`) keeps a 40 B `{num, last_heard, public_key}` record per evicted node - primarily so PKI DMs to/from a long-tail node keep decrypting without re-running a NodeInfo exchange (the rest of `NodeInfoLite` rebuilds from traffic in seconds).
...
- **Persistence:** nRF52840 uses a 12 KB raw-flash record-ring at `0xEA000` (below LittleFS; append + replay + compact-on-rotate, link-guarded by `nrf52840_s140_v7.ld` and `extra_scripts/nrf52_warm_region.py`). Everywhere else, nRF54L15 included: a `/prefs/warm.dat` snapshot flushed by `saveIfDirty()` on the node-DB save cadence.
