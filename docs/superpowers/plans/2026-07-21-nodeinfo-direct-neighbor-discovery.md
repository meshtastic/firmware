...
- Broadcast NodeInfo requests that ask for replies must be emitted at hop limit zero.
...
- [ ] **Step 1: Write failing tests** - Build direct (`hop_start == hop_limit`), relayed (`hop_limit < hop_start`), unknown-hop (zero hop start and no decoded bitfield), and unicast NodeInfo requests. Assert only direct broadcast qualifies and its reply hop limit is zero; assert `sendOurNodeInfo(NODENUM_BROADCAST, true)` sets `hop_limit` to zero; unicast retains the routing default.
