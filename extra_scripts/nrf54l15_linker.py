...
if env.get("PIOENV") != "nrf54l15dk":
    ...
    def _generate_linker_cmd(target, source, env):
        ...
        if not os.path.exists(ninja_build):
            raise RuntimeError(
                ...
                "Run a full build first so CMake generates the Ninja files." % ninja_build
            )

        gcc_cmd, cwd = _extract_gcc_command(ninja_build)
        run_cwd = cwd if cwd else zephyr_dir

        print("==> nRF54L15: Generating zephyr/linker.cmd (LINKER_ZEPHYR_FINAL) via GCC")
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(
                "nRF54L15 linker fix: GCC failed to generate linker.cmd (rc=%d)" % result.returncode
            )
    ...
    if prog:
        ...
        print("[nrf54l15_linker] WARNING: PIOMAINPROG not set, falling back to $PROG_PATH")
