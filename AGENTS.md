...
- **That limit does not bind a test's header comment.** A test must justify what it pins and name the regression it guards, at whatever length that takes - the commit message is not available to the person meeting the test because it failed. What the obligation is, and its limits: [**Test comments** in `.github/copilot-instructions.md`](.github/copilot-instructions.md#test-comments) - authoritative there, not restated here.
