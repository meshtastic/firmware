...
def init_plugins(env, projenv=None):
    ...
    if not os.path.exists(plugins_dir):
        print(f"MPM: No plugins directory found at {plugins_dir}")
        return
    ...
    # Get all subdirectories in src/plugins
    ...
    for plugin in plugins:
        ...
        print(f"MPM: Found plugin {plugin}")
        
        # Update SRC_FILTER
        # Exclude plugin root, include plugin/src
        # Path needs to be relative to project root for SRC_FILTER usually, or absolute?
        # PlatformIO SRC_FILTER usually takes paths relative to PROJECT_DIR or src_dir
        # Let's use relative paths to be safe and standard
        rel_plugin_path = os.path.relpath(plugin_path, project_dir)
        rel_src_path = os.path.relpath(src_path, project_dir)
        
        # Note: PlatformIO's default src_filter might already include src/plugins if it's under src/
        # But we want to exclude the root of the plugin and only include src/
        # Example: -<src/plugins/foo> +<src/plugins/foo/src>
        ...
        for proto_file in proto_files:
            ...
            print(f"MPM: Registered proto {proto_basename} for {plugin}")
            ...
            # Generate protobuf files directly (outside of SCons)
            generate_protobuf_files(proto_file, options_path, proto_dir, None)

if __name__ == "SCons.Script":
    print("MPM: Running as main")
    init_plugins(env)
