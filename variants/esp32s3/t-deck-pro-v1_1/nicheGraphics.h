...
NicheGraphics setup for LILYGO T-Deck Pro V1.1 (GDEQ031T10 3.1" 240x320 on the default SPI bus).

Same e-ink controller as the original T-Deck Pro, but V1.1 breaks out the panel reset pin
(PIN_EINK_RES). The base SSD16XX driver deep-sleeps the panel after a full refresh whenever a
reset pin is present; the original GxEPD2 build set EINK_NOT_HIBERNATE to avoid panel
wake/refresh quirks on this revision, so we preserve that by overriding deepSleep() to a no-op.
...
// GDEQ031T10, but skip the controller deep-sleep (preserves the V1.1 EINK_NOT_HIBERNATE behaviour).
// Deep-sleep only saves a few µA, so suppressing it is a safe trade for panel compatibility.
