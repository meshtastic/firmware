...
namespace
{
constexpr uint16_t MOTOR_POWER_HOLD_MS = 250;
}

bool TDeckProHapticOutput::begin()
{
    ...
    if (!driver.begin()) {
        ...
        LOG_ERROR("T-Deck-Pro: DRV2605 initialization failed");
        return false;
    }
...
void TDeckProHapticOutput::play(HapticEffect effect)
{
...
void TDeckProHapticOutput::triggerLocked(HapticEffect effect)
{
    if (!ready || effect == HapticEffect::NONE)
        return;
...
void TDeckProHapticOutput::stop()
{
    concurrency::LockGuard guard(&driverLock);
    if (!ready)
        return;
...
void TDeckProHapticOutput::stopDriverLocked()
{
    driver.stop();
    pulseActive = false;
    if (motorPowerOn)
...
void TDeckProHapticOutput::pulse(uint16_t durationMs)
{
    concurrency::LockGuard guard(&driverLock);
    if (!ready)
        return;
...
void TDeckProHapticOutput::armDelayedPulse(uint16_t delayMs, uint16_t durationMs)
{
    concurrency::LockGuard guard(&driverLock);
    if (!ready)
        return;
...
void TDeckProHapticOutput::cancelDelayedPulse()
{
    concurrency::LockGuard guard(&driverLock);
    delayedPulsePending = false;
}

int32_t TDeckProHapticOutput::runOnce()
{
    concurrency::LockGuard guard(&driverLock);
    ...
    if (!Throttle::isWithinTimespanMs(motorPowerChangedAt, MOTOR_POWER_HOLD_MS)) {
        power->setMotorPower(false);
        motorPowerOn = false;
    }
