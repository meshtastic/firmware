...
TDeckMaxBHI260APSensor::TDeckMaxBHI260APSensor(ScanI2C::FoundDevice foundDevice)
    : MotionSensor::MotionSensor(foundDevice)
{
}

TDeckMaxBHI260APSensor::~TDeckMaxBHI260APSensor()
{
    releaseResources();
}

TDeckMaxBHI260APSensor *TDeckMaxBHI260APSensor::interruptInstance = nullptr;

void TDeckMaxBHI260APSensor::releaseResources()
{
    if (irqAttached) {
    ...
    if (auto *controller = getDevicePowerController())
        controller->setImuPower(false);
}

void TDeckMaxBHI260APSensor::accelerationCallback(uint8_t sensorId, uint8_t *data, uint32_t length,
                                                  uint64_t *timestamp, void *userData)
{
    (void)timestamp;
    auto *instance = static_cast<TDeckMaxBHI260APSensor *>(userData);
    if (instance)
        instance->processAcceleration(sensorId, data, length);
}

void TDeckMaxBHI260APSensor::processAcceleration(uint8_t sensorId, uint8_t *data, uint32_t length)
{
...
void TDeckMaxBHI260APSensor::dataReadyISR()
{
    if (interruptInstance)
        interruptInstance->dataReadyPending = true;
}

bool TDeckMaxBHI260APSensor::init()
{
    releaseResources();
    if (auto *controller = getDevicePowerController())
        controller->setImuPower(true);
...
int32_t TDeckMaxBHI260APSensor::runOnce()
{
    if (!initialized)
