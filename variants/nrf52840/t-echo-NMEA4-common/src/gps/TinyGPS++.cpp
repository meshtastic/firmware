...
bool TinyGPSPlus::encode(char c)
{
    ...
    case '$': // sentence begin
        ...
        pendingGSAUsed = 0;
        memset(pendingGSAIds, 0, sizeof(pendingGSAIds));
        pendingGSAFixType = 0;
        pendingGSAPDOP = 0;
        pendingGSAHDOP = 0;
        pendingGSAVDOP = 0;
...
int TinyGPSPlus::GGA(char *buf)
{
    ...
    if (fixQuality == 0) {
            ...
            sprintf(end, "$_GGA,%02d%02d%02d.%02d,%02d%10.7f,%c,%03d%10.7f,%c,%d,%02d,%.1f,%.3f,M,%.3f,M,,", time.hour(),
                    time.minute(), time.second(), time.centisecond(), location.rawLat().deg,
                    (location.lat() - location.rawLat().deg) * 60, location.rawLat().negative ? 'S' : 'N', location.rawLng().deg,
                    (location.lng() - location.rawLng().deg) * 60, location.rawLng().negative ? 'W' : 'E', fixQuality,
                    satellites.value(), hdop.hdop(), altitude.meters(), geoidHeight.meters());
    }
...
bool TinyGPSPlus::endOfTermHandler(bool termIsNotEmpty)
{
    ...
    if ((flags & FLAG_IS_CHECKSUM_TERM) != 0) {
        ...
        if (checksum == parity) {
            ...
            case GPS_SENTENCE_VTG:
                // VTG is intentionally independent from the primary RMC data. Commit
                // only checksum-valid navigation data and preserve the last valid VTG
                // sample if a later VTG sentence is corrupt. Mode N means no valid fix.
                vtgInfo.mode = pendingVTGMode;
                vtgInfo.valid = pendingVTGMode != 'N' && vtgCourse.isNotEmpty() && vtgSpeed.isNotEmpty();
                vtgInfo.lastUpdate = sentenceTime;
                if (vtgInfo.valid) {
            ...
            case GPS_SENTENCE_ZDA:
                ...
                zdaInfo.valid = pendingZDAYear >= 2000 && pendingZDAMonth >= 1 && pendingZDAMonth <= 12 && pendingZDADay >= 1 &&
                                pendingZDADay <= 31;
    ...
    if (curTermNumber == 0) {
        ...
        else if (strlen(term) == 5 && !strncmp(term + 2, "VTG", 3))
            curSentenceType = GPS_SENTENCE_VTG;
    ...
    if (curSentenceType == GPS_SENTENCE_ZDA) {
        ...
        case 1:
            ...
            if (termIsNotEmpty)
                zdaTime.setTime(term);
            break;
        case 2:
            if (termIsNotEmpty)
                pendingZDADay = (uint8_t)atoi(term);
            break;
        case 3:
            if (termIsNotEmpty)
                pendingZDAMonth = (uint8_t)atoi(term);
            break;
        case 4:
            if (termIsNotEmpty)
                pendingZDAYear = (uint16_t)atoi(term);
            break;
        case 5:
            if (termIsNotEmpty)
                pendingZDAZoneHours = (int8_t)atoi(term);
            break;
        case 6:
            if (termIsNotEmpty)
                pendingZDAZoneMinutes = (uint8_t)atoi(term);
            break;
        }
    }
    ...
    if (curSentenceType == GPS_SENTENCE_GSA) {
        if (curTermNumber == 2 && termIsNotEmpty) {
            ...
            const uint8_t slot = (uint8_t)(curTermNumber - 3);
            if (termIsNotEmpty) {
                pendingGSAIds[slot] = (uint16_t)atoi(term);
                ++pendingGSAUsed;
            }
        } else if (curTermNumber == 15 && termIsNotEmpty)
            pendingGSAPDOP = (uint16_t)parseDecimal(term);
        else if (curTermNumber == 16 && termIsNotEmpty)
            pendingGSAHDOP = (uint16_t)parseDecimal(term);
            ...
            case 1:
                ...
                break;
            case 2:
                pendingGSASystem = TINYGPS_GNSS_GLONASS;
                break;
            case 3:
                pendingGSASystem = TINYGPS_GNSS_GALILEO;
                break;
            case 4:
                pendingGSASystem = TINYGPS_GNSS_BEIDOU;
                break;
            case 5:
                pendingGSASystem = TINYGPS_GNSS_QZSS;
                break;
            default:
                pendingGSASystem = TINYGPS_GNSS_UNKNOWN;
                break;
    ...
    if (curSentenceType == GPS_SENTENCE_GSV) {
        if (curTermNumber == 1 && termIsNotEmpty) {
            ...
            if (currentGSVMessageNumber == 1 && (currentGSVSystem != TINYGPS_GNSS_MIXED || !gsvHasSignalId)) {
                for (size_t i = 0; i < TINYGPS_MAX_SATS; ++i) {
                    if (trackedSatellites[i].system == currentGSVSystem ||
                        (currentGSVTalkerIsGP && trackedSatellites[i].system == TINYGPS_GNSS_QZSS))
            ...
            if (currentGSVMessageNumber > 0 && currentGSVTotalMessages > 0) {
                if (currentGSVMessageNumber < currentGSVTotalMessages) {
                    ...
                    int remaining = (int)currentGSVTotalSatellites - ((int)currentGSVMessageNumber - 1) * 4;
                    if (remaining < 0)
                        remaining = 0;
                    if (remaining > 4)
                        remaining = 4;
            ...
            const bool isSignalIdTerm = satellitesThisSentence > 0 && curTermNumber == signalIdTerm && termIsNotEmpty;
            ...
            if (isSignalIdTerm) {
                ...
                case 16: {
                    ...
                    if (termIsNotEmpty) {
                        ...
                        // Quectel L76K follows NMEA convention where GP is used for both
                        // GPS and QZSS. PRNs 193..197 therefore identify QZSS even though
                        // the sentence talker is GP.
                        uint8_t satelliteSystem = currentGSVSystem;
                        if (currentGSVTalkerIsGP && prn >= 193 && prn <= 197)
                            satelliteSystem = TINYGPS_GNSS_QZSS;

                        int freeSlot = -1;

                        for (size_t i = 0; i < TINYGPS_MAX_SATS; ++i) {
                            if (trackedSatellites[i].prn == prn && trackedSatellites[i].system == satelliteSystem) {
    ...
    } else if (curSentenceType == GPS_SENTENCE_VTG) {
        // L76K VTG: COG true (term 1), SOG knots (term 5), SOG km/h
        // (term 7), ModeInd (term 9). RMC remains primary; VTG is stored
        // separately so it can never overwrite a good RMC navigation sample.
        switch (curTermNumber) {
        case 1:
            vtgCourse.setNotEmpty(termIsNotEmpty);
            if (termIsNotEmpty)
                vtgCourse.set(term);
            break;
        case 5:
            vtgSpeed.setNotEmpty(termIsNotEmpty);
            if (termIsNotEmpty)
                vtgSpeed.set(term);
            break;
        case 9:
        ...
        case COMBINE(GPS_SENTENCE_RMC, 2): // GPRMC validity
            setSentenceHasFix(term[0] == 'A' || term[0] == 'D');
            break;
