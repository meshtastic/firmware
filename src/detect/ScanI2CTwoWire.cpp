...
bool ScanI2CTwoWire::i2cCommandResponseLength(DeviceAddress addr, uint16_t command, uint8_t expectedLength) const
{
    TwoWire *i2cBus = fetchI2CBus(addr);
    uint8_t cmd[2] = {static_cast<uint8_t>((command >> 8) & 0xFF), static_cast<uint8_t>(command & 0xFF)};
    ...
    delay(20);
    uint8_t receivedLength = i2cBus->requestFrom(addr.address, expectedLength);
