...
#endif
#if defined(HAS_QMA6100P) &&                                                   \
    (defined(ARCH_NRF52) || defined(NRF52_SERIES) || defined(NRF52))
...
bool probeQMA6100P(uint8_t address) {
  ...
  const bool readOk =
      Nrf52Twim::readRegister(address, SFE_QMA6100P_CHIP_ID, chipID);
...
ScanI2C::FoundDevice ScanI2CTwoWire::find(ScanI2C::DeviceType type) const {
  ...
  return exists(type) ? ScanI2C::FoundDevice(type, deviceAddresses.at(type))
                      : DEVICE_NONE;
}
...
ScanI2C::FoundDevice ScanI2CTwoWire::firstOfOrNONE(size_t count,
                                                   DeviceType types[]) const {
  concurrency::LockGuard guard((concurrency::Lock *)&lock);
...
ScanI2C::DeviceType
ScanI2CTwoWire::probeOLED(ScanI2C::DeviceAddress addr) const {
...
uint16_t ScanI2CTwoWire::getRegisterValue(
  ...
  LOG_DEBUG("Register value from 0x%x: 0x%x",
            registerLocation.i2cAddress.address, value);
  return value;
}

bool ScanI2CTwoWire::i2cCommandResponseLength(ScanI2C::DeviceAddress addr,
                                              uint16_t command,
                                              uint8_t expectedLength) const {
  TwoWire *i2cBus = fetchI2CBus(addr);
  i2cBus->beginTransmission(addr.address);
  if (command > 0xFF) {
...
bool detectSHT21SerialNumber(TwoWire *i2cBus, uint8_t address) {
  ...
  return crcSHT2X(&serialA[0], 1) == serialA[1] &&
         crcSHT2X(&serialA[2], 1) == serialA[3] &&
         crcSHT2X(&serialA[4], 1) == serialA[5] &&
         crcSHT2X(&serialA[6], 1) == serialA[7] &&
         crcSHT2X(&serialB[0], 2) == serialB[2] &&
         crcSHT2X(&serialB[3], 2) == serialB[5];
}
#endif
  ...
  for (addr.address = 8; addr.address < 120; addr.address++) {
...
#if defined(HAS_QMA6100P) &&                                                   \
    (defined(ARCH_NRF52) || defined(NRF52_SERIES) || defined(NRF52))
    if (addr.address == QMA6100P_ADDRESS_LOW ||
        addr.address == QMA6100P_ADDRESS_HIGH) {
      ...
      err = 2;
      if ((addr.address >= 0x30 && addr.address <= 0x37) ||
          (addr.address >= 0x50 && addr.address <= 0x5F)) {
        if (i2cBus->read() != -1)
          err = 0;
      } else {
    ...
    if (err == 0) {
...
#ifdef PCF8563_RTC
        SCAN_SIMPLE_CASE(PCF8563_RTC, RTC_PCF8563, "PCF8563",
                         (uint8_t)addr.address)
#endif
#ifdef RX8130CE_RTC
        SCAN_SIMPLE_CASE(RX8130CE_RTC, RTC_RX8130CE, "RX8130CE",
                         (uint8_t)addr.address)
#endif

#ifdef PCF85063_RTC
        SCAN_SIMPLE_CASE(PCF85063_RTC, RTC_PCF85063, "PCF85063",
                         (uint8_t)addr.address)
#endif
      ...
      case CARDKB_ADDR:
        ...
        registerValue =
            getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x04), 1);
...
#ifdef HAS_NCP5623
        SCAN_SIMPLE_CASE(NCP5623_ADDR, NCP5623, "NCP5623",
                         (uint8_t)addr.address);
#endif
      case XPOWERS_AXP192_AXP2101_ADDRESS:
        // Do we have the axp2101/192 or the TCA8418
        registerValue =
            getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x90), 1);
      ...
      case BME_ADDR_ALTERNATE:
        registerValue = getRegisterValue(
            ScanI2CTwoWire::RegisterLocation(addr, 0xD0), 1); // GET_ID
        ...
        case 0x00:
          ...
          registerValue =
              getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x0D), 1);
          switch (registerValue) {
          case 0x10:
        ...
        default:
          registerValue = getRegisterValue(
              ScanI2CTwoWire::RegisterLocation(addr, 0x00), 1); // GET_ID
          switch (registerValue) {
      ...
      case AHT10_ADDR:
        ...
        if (mfg == 0x5449 || mfg == 0x190F) {
          uint16_t die =
              getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0xFF), 2);
      ...
      case MCP9808_ADDR:
        ...
        // 0x00 doesn't seems to be colliding with MCP9808 and LIS3DH chips.
        {
          ...
          registerValue =
              getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x00), 2);
      ...
      case RCWL9620_ADDR:
        // get MAX30102 PARTID
        registerValue =
            getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0xFF), 1);
        ...
        break;
            ...
            case HMC5883L_ADDR:
                ...
                break;
#ifdef HAS_QMA6100P
        SCAN_SIMPLE_CASE(QMA6100P_ADDR, QMA6100P, "QMA6100P",
                         (uint8_t)addr.address)
#else
        SCAN_SIMPLE_CASE(PMSA003I_ADDR, PMSA003I, "PMSA003I",
                         (uint8_t)addr.address)
#endif
      case MMC5983MA_ADDR:
        registerValue =
            getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x2F), 1);
        ...
        SCAN_SIMPLE_CASE(LSM6DS3_ADDR, LSM6DS3, "LSM6DS3",
                         (uint8_t)addr.address);
        SCAN_SIMPLE_CASE(VEML7700_ADDR, VEML7700, "VEML7700",
                         (uint8_t)addr.address);
      case TCA9555_ADDR:
        registerValue =
            getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x01), 1);
        if (registerValue == 0x13) {
          registerValue =
              getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x00), 1);
      ...
      case TSL25911_ADDR:
        registerValue = getRegisterValue(
            ScanI2CTwoWire::RegisterLocation(addr, 0xA0 | 0x12), 1);
      ...
      case LTR553ALS_ADDR:
        registerValue =
            getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x86),
                             1);     // Part ID register
        if (registerValue == 0x92) { // LTR553ALS Part ID
          ...
          if (bh1750_error == 0) {
            ...
            LOG_INFO(
                "Device found at address 0x%x was not able to be enumerated",
                (uint8_t)addr.address);
          }
        }
        break;

        SCAN_SIMPLE_CASE(BHI260AP_ADDR, BHI260AP, "BHI260AP",
                         (uint8_t)addr.address);
      case SCD4X_ADDR: {
        registerValue =
            getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x8), 1);
    ...
    (defined(ARCH_NRF52) || defined(NRF52_SERIES) || defined(NRF52))
        ...
        break;
#ifdef HAS_TPS65233
        SCAN_SIMPLE_CASE(TPS65233_ADDR, TPS65233, "TPS65233",
                         (uint8_t)addr.address);
#endif
      ...
      case MLX90614_ADDR_DEF:
        ...
        registerValue =
            getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x06), 1);
        if (registerValue == 0xAB) {
          type = CST226SE;
          logFoundDevice("CST226SE", (uint8_t)addr.address);
        } else if (getRegisterValue(
                       ScanI2CTwoWire::RegisterLocation(addr, 0x0e), 1) ==
                   0x5a) {
          type = MLX90614;
          logFoundDevice("MLX90614", (uint8_t)addr.address);
        } else {
          registerValue =
              getRegisterValue(ScanI2CTwoWire::RegisterLocation(addr, 0x00),
      ...
      case ICM20948_ADDR_ALT: // same as MPU6050_ADDR, BMI270_ADDR, and
        ...
        if (registerValue == 0xEA) {
          ...
          registerValue = getRegisterValue(
              ScanI2CTwoWire::RegisterLocation(addr, 0x75), 1); // WHO_AM_I
      ...
      case CGRADSENS_ADDR:
        ...
        if (registerValue == 0x7D || registerValue == 0x7A) {
          ...
          LOG_DEBUG("Unexpected Device ID for RadSense: addr=0x%x id=0x%x",
                    CGRADSENS_ADDR, registerValue);
        }
        break;
...
void ScanI2CTwoWire::scanPort(I2CPort port) { scanPort(port, nullptr, 0); }
