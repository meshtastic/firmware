    ...
    case meshtastic_Config_security_tag:
#ifdef MESHTASTIC_ENCRYPTED_STORAGE
        // set_config(security) doubles as the lockdown passphrase transport (no schema changes).
        ...
        //   admin_key[2].bytes[0..3] LE u32 — valid_until_epoch (absolute Unix timestamp; 0 → no time limit)
        {
            ...
            if (ppLen == 1 && pp[0] == 0xFF) {
                LOG_INFO("AdminModule: lockdown LOCK NOW command received");
            ...
            if (ppLen >= 1 && ppLen <= 32) {
                ...
                if (!EncryptedStorage::isUnlocked()) {
                    ...
                    if (!EncryptedStorage::isProvisioned()) {
                        LOG_INFO("AdminModule: lockdown first-time provisioning with passphrase");
                        ok = EncryptedStorage::provisionPassphrase(pp, ppLen, boots, validUntilEpoch);
                    } else {
                        LOG_INFO("AdminModule: lockdown unlock with passphrase");
                    ...
                    if (ok) {
                        ...
                        {
                            char unlockedMsg[64];
                            snprintf(unlockedMsg, sizeof(unlockedMsg), "LOCKDOWN_UNLOCKED:boots=%d:until=%u",
                                     (int)EncryptedStorage::getBootsRemaining(),
                                     (unsigned)EncryptedStorage::getValidUntilEpoch());
                            sendWarning(unlockedMsg);
                        }
                        LOG_INFO("AdminModule: lockdown storage unlocked, config reloaded");
                    }
                } else {
                    ...
                    LOG_INFO("AdminModule: lockdown passphrase re-verify for admin authorization");
                    ok = EncryptedStorage::unlockWithPassphrase(pp, ppLen, boots, validUntilEpoch);
                    if (ok) {
#ifdef MESHTASTIC_PHONEAPI_ACCESS_CONTROL
                        PhoneAPI::authorizeLocalAdmin();
                        ...
                        LOG_INFO("AdminModule: lockdown passphrase verified, local admin authorized");
                    }
                }
                ...
                if (!ok) {
                    ...
                    char failMsg[64];
                    if (backoff > 0)
                        snprintf(failMsg, sizeof(failMsg), "LOCKDOWN_UNLOCK_FAILED:backoff=%u", backoff);
                    else
                        snprintf(failMsg, sizeof(failMsg), "LOCKDOWN_UNLOCK_FAILED:wrong_passphrase");
                    sendWarning(failMsg);
                    LOG_WARN("AdminModule: lockdown passphrase verification failed");
                }
