...
#endif
#include "modules/PhoneAPIModule.h"
...
void setupModules() {
...
#endif
#if !MESHTASTIC_EXCLUDE_DETECTIONSENSOR
  if (moduleConfig.has_detection_sensor &&
      moduleConfig.detection_sensor.enabled) {
  ...
  if (moduleConfig.has_telemetry &&
      ...
      nodeTelemetrySensorsMap[meshtastic_TelemetrySensorType_PMSA003I].first >
          0) {
    new AirQualityTelemetryModule();
  }
#endif
#if !MESHTASTIC_EXCLUDE_HEALTH_TELEMETRY
  if (nodeTelemetrySensorsMap[meshtastic_TelemetrySensorType_MAX30102].first >
          0 ||
      nodeTelemetrySensorsMap[meshtastic_TelemetrySensorType_MLX90614].first >
          0) {
  ...
  if (moduleConfig.has_telemetry &&
      (moduleConfig.telemetry.power_measurement_enabled ||
       moduleConfig.telemetry.power_screen_enabled)) {
  ...
  if (moduleConfig.has_range_test && moduleConfig.range_test.enabled)
    new RangeTestModule();
#endif
  // NOTE! PhoneAPIModule handles packet delivery to connected phone/API clients
  phoneAPIModule = new PhoneAPIModule();
