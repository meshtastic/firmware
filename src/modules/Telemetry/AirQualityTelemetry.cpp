...
#else
NullSensor sfa30Sensor;
#endif

int32_t AirQualityTelemetryModule::runOnce() {
  if (sleepOnNextExecution == true) {
    sleepOnNextExecution = false;
    uint32_t nightyNightMs = Default::getConfiguredOrDefaultMs(
  ...
  */
  ...
  // TODO there is no config in module_config.proto for
  // air_quality_screen_enabled. Reusing environment one, although it should
  // have its own
  ...
  if (firstTime) {
    ...
    if (moduleConfig.telemetry.air_quality_enabled) {
      ...
      if (sfa30Sensor.hasSensor()) {
        result = sfa30Sensor.runOnce();
        LOG_DEBUG("Initialised SFA30!");
      }
    }
    LOG_INFO("Init: %i", result);
    ...
    if (((lastSentToMesh == 0) ||
      ...
      lastSentToMesh = millis();
                ...
                !Throttle::isWithinTimespanMs(lastSentToPhone,
...
void AirQualityTelemetryModule::drawFrame(OLEDDisplay *display,
  ...
  bool hasAny = m.has_pm10_standard || m.has_pm25_standard ||
                m.has_pm100_standard || m.has_pm10_environmental ||
...
bool AirQualityTelemetryModule::handleReceivedProtobuf(
    const meshtastic_MeshPacket &mp, meshtastic_Telemetry *t) {
  if (t->which_variant == meshtastic_Telemetry_air_quality_metrics_tag) {
    ...
    LOG_INFO("                  | PM1.0(Environmental)=%i, "
             ...
             t->variant.air_quality_metrics.pm10_environmental,
             t->variant.air_quality_metrics.pm25_environmental,
...
bool AirQualityTelemetryModule::getAirQualityTelemetry(
    meshtastic_Telemetry *m) {
  LOG_DEBUG("getAirQualityTelemetry");
  bool valid = true;
  ...
  if (sfa30Sensor.hasSensor()) {
    valid = valid && sfa30Sensor.getMetrics(m);
    hasSensor = true;
  }
...
bool AirQualityTelemetryModule::sendTelemetry(NodeNum dest, bool phoneOnly) {
  ...
  m.time = getTime();
  if (getAirQualityTelemetry(&m)) {
    LOG_INFO("Send: pm10_standard=%f, pm25_standard=%f, pm100_standard=%f, "
             "pm10_environmental=%f, pm100_environmental=%f",
             m.variant.air_quality_metrics.pm10_standard,
             m.variant.air_quality_metrics.pm25_standard,
    ...
    if (phoneOnly) {
      ...
      if (config.device.role == meshtastic_Config_DeviceConfig_Role_SENSOR &&
        ...
        meshtastic_ClientNotification *notification =
            clientNotificationPool.allocZeroed();
        notification->level = meshtastic_LogRecord_Level_INFO;
        notification->time = getValidTime(RTCQualityFromNet);
        sprintf(notification->message,
                "Sending telemetry and sleeping for %us interval in a moment",
...
AdminMessageHandleResult AirQualityTelemetryModule::handleAdminMessageForModule(
    const meshtastic_MeshPacket &mp, meshtastic_AdminMessage *request,
