...
// #define RST_PIN_MFRC522 5
#define MFRC522_I2C_ADDRESS (0x28) // Primary I2C Address
/////////////////////////////////////////////////////////////////////////////////////
...
MFRC522_I2C::MFRC522_I2C(TwoWire *TwoWireInstance)
{
	_chipAddress = MFRC522_I2C_ADDRESS;
	_resetPowerDownPin = 5;
	_TwoWireInstance = TwoWireInstance;
}
MFRC522_I2C::~MFRC522_I2C(void)
{
	if (i2c_dev)
		delete i2c_dev;
}

bool MFRC522_I2C::begin_I2C(TwoWire *theWire)
{
	// Serial.print("!i2c_dev" + !i2c_dev); //
	// Serial.println("i2c_dev_start"); //
	if (i2c_dev)
	{
		delete i2c_dev; // remove old interface
	}

	i2c_dev = new Adafruit_I2CDevice(MFRC522_I2C_ADDRESS, theWire);
...
// void MFRC522_I2C::PCD_WriteRegister(byte reg, byte count, byte *values)
// {
// 	if (count == 0)
// 	{
// 		return;
...
// 	//   Adafruit_I2CDevice i2c_dev(_TwoWireInstance, _chipAddress);
...
// 	// buffer[0] = reg;
// 	uint8_t readValue;
// 	i2c_dev->write_then_read(&regAddr, 1, &readValue, 1);
// 	Serial.print("Value written to register 0x");
...
{
	Serial.println("  ");
	Serial.println("PCD_Init()_start");
	...
	Serial.println("Делаем PCD_Reset");
	PCD_Reset();
...
{
	Serial.println("  ");
	Serial.println("PCD_Reset()_start");
...
{
	Serial.println("  ");
	Serial.println("PICC_IsNewCardPresent()_start");
	byte bufferATQA[2];
	byte bufferSize = sizeof(bufferATQA);
	byte result = PICC_RequestA(bufferATQA, &bufferSize);
	Serial.println("PICC_IsNewCardPresent()_end");
...
{
	Serial.println("  ");
	Serial.println("PICC_ReadCardSerial()_start");
	byte result = PICC_Select(&uid);
	Serial.println("PICC_ReadCardSerial()_end");
