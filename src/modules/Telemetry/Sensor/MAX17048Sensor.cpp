...
MAX17048Sensor::MAX17048Sensor()
    : TelemetrySensor(meshtastic_TelemetrySensorType_MAX17048, "MAX17048") {}
...
bool MAX17048Sensor::getMetrics(meshtastic_Telemetry *measurement) {
  ...
  LOG_DEBUG(
      "MAX17048Sensor::getMetrics volts: %.3fV soc: %.1f%% ttg: %.1f hours\n",
      volts, soc, ttg);
  if ((int)measurement->which_variant ==
      meshtastic_Telemetry_power_metrics_tag) {
    measurement->variant.power_metrics.has_ch1_voltage = true;
    measurement->variant.power_metrics.ch1_voltage = volts;
  } else if ((int)measurement->which_variant ==
             meshtastic_Telemetry_device_metrics_tag) {
    measurement->variant.device_metrics.has_battery_level = true;
    measurement->variant.device_metrics.has_voltage = true;
    measurement->variant.device_metrics.battery_level =
        static_cast<uint32_t>(round(soc));
...
uint8_t MAX17048Sensor::getBusBatteryPercent() {
  ...
  return clamp(static_cast<uint8_t>(round(soc)), static_cast<uint8_t>(0),
...
uint16_t MAX17048Sensor::getTimeToGoSecs() {
  ...
  float soc = max17048->cellPercent(); // state of charge in percent 0 to 100
  soc = clamp(soc, 0.0f, 100.0f);      // clamp soc between 0 and 100%
  float ttg = ((100.0f - soc) / rate) *
              3600.0f; // calculate seconds to charge/discharge
...
bool MAX17048Singleton::runOnce(TwoWire *theWire) { return begin(theWire); }
...
bool MAX17048Singleton::isBatteryCharging() {
  ...
  if (chargeSamples.front().chargeRate > MAX17048_CHARGING_MINIMUM_RATE &&
    ...
    else if (chargeSamples.front().cellPercent <
             chargeSamples.back().cellPercent)
