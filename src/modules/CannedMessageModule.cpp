...
CannedMessageModule::CannedMessageModule() : SinglePortModule("canned", meshtastic_PortNum_TEXT_MESSAGE_APP), concurrency::OSThread("CannedMessage") {
  this->loadProtoForModule();
  if ((this->splitConfiguredMessages() <= 0) && (cardkb_found.address == 0x00) && !INPUTBROKER_MATRIX_TYPE && !CANNED_MESSAGE_MODULE_ENABLE) {
...
bool hasKeyForNode(const meshtastic_NodeInfoLite *node) { return node && node->has_user && node->user.public_key.size > 0; }
/**
...
bool CannedMessageModule::isUpEvent(const InputEvent *event) {
  return event->inputEvent == INPUT_BROKER_UP || ((runState == CANNED_MESSAGE_RUN_STATE_ACTIVE || runState == CANNED_MESSAGE_RUN_STATE_EMOTE_PICKER ||
...
bool CannedMessageModule::isSelectEvent(const InputEvent *event) { return event->inputEvent == INPUT_BROKER_SELECT; }
...
bool CannedMessageModule::handleMessageSelectorInput(const InputEvent *event, bool isUp, bool isDown, bool isSelect) {
  ...
  if (runState != CANNED_MESSAGE_RUN_STATE_INACTIVE && (event->inputEvent == INPUT_BROKER_CANCEL || event->inputEvent == INPUT_BROKER_ALT_LONG)) {
...
bool CannedMessageModule::handleFreeTextInput(const InputEvent *event) {
  ...
  bool isSelect = isSelectEvent(event);
  if (isSelect) {
    LOG_DEBUG("[SELECT] handleFreeTextInput: runState=%d, dest=%u, channel=%d, freetext='%s'", (int)runState, dest, channel, freetext.c_str());
    if (dest == 0)
      dest = NODENUM_BROADCAST;
...
int32_t CannedMessageModule::runOnce() {
  ...
  UIFrameEvent e;
  if ((this->runState == CANNED_MESSAGE_RUN_STATE_SENDING_ACTIVE && this->payload != 0 && this->payload != CANNED_MESSAGE_RUN_STATE_FREETEXT) ||
      ...
      default:
        ...
        if (this->payload >= 32 && this->payload <= 126) {
          ...
          if (this->cursor == this->freetext.length()) {
            ...
            this->freetext = this->freetext.substring(0, this->cursor) + (char)this->payload + this->freetext.substring(this->cursor);
          }
          this->cursor++;
...
const char *CannedMessageModule::getCurrentMessage() { return this->messages[this->currentMessageIndex]; }
const char *CannedMessageModule::getPrevMessage() { return this->messages[this->getPrevIndex()]; }
const char *CannedMessageModule::getNextMessage() { return this->messages[this->getNextIndex()]; }
const char *CannedMessageModule::getMessageByIndex(int index) { return (index >= 0 && index < this->messagesCount) ? this->messages[index] : ""; }
...
bool CannedMessageModule::shouldDraw() {
  ...
  return (this->runState == CANNED_MESSAGE_RUN_STATE_ACTIVE || this->runState == CANNED_MESSAGE_RUN_STATE_FREETEXT ||
          this->runState == CANNED_MESSAGE_RUN_STATE_DESTINATION_SELECTION || this->runState == CANNED_MESSAGE_RUN_STATE_EMOTE_PICKER);
}

// Has the user defined any canned messages?
// Expose publicly whether canned message module is ready for use
bool CannedMessageModule::hasMessages() { return (this->messagesCount > 0); }
...
String CannedMessageModule::keyForCoordinates(uint x, uint y) {
  ...
  for (int8_t outerIndex = 0; outerIndex < outerSize; outerIndex++) {
    ...
    for (int8_t innerIndex = 0; innerIndex < innerSize; innerIndex++) {
      ...
      if (x > letter.rectX && x < (letter.rectX + letter.rectWidth) && y > letter.rectY && y < (letter.rectY + letter.rectHeight)) {
    ...
    for (int8_t innerIndex = 0; innerIndex < innerSize; innerIndex++) {
      ...
      if (letter.character == "⇧") {
        ...
        if (this->highlight == letter.character[0]) {
          ...
          display->drawString(xOffset + characterOffset, yOffset + yCorrection, letter.character == " " ? "space" : letter.character);
  ...
  for (int row = 0; row < visibleRows; row++) {
    ...
    // Then Draw Nodes
      ...
      if (nodeIndex >= 0 && nodeIndex < static_cast<int>(this->filteredNodes.size())) {
        ...
        int availWidth = display->getWidth() - ((graphics::currentResolution == graphics::ScreenResolution::High) ? 40 : 20) -
...
void CannedMessageModule::drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) {
  ...
  if (this->messagesCount > 0) {
    ...
    topMsg = (messagesCount > _visibleRows && currentMessageIndex >= _visibleRows - 1) ? currentMessageIndex - _visibleRows + 2 : 0;
      ...
      mp.decoded.request_id == this->lastRequestId) // only ACKs for our last sent packet
  {
    if (mp.decoded.request_id != 0) {
      ...
      if (screen) {
        ...
        int availWidth = display->getWidth() - ((graphics::currentResolution == graphics::ScreenResolution::High) ? 60 : 30);
        if (availWidth < 0)
          availWidth = 0;
        ...
        if (this->ack) {
          if (this->lastSentNode == NODENUM_BROADCAST) {
            snprintf(buf, sizeof(buf), "Message sent to\n#%s\n\nSignal: %s", (channelName && channelName[0]) ? channelName : "unknown", qualityLabel);
          } else {
            snprintf(buf, sizeof(buf), "DM sent to\n@%s\n\nSignal: %s", (nodeName && nodeName[0]) ? nodeName : "unknown", qualityLabel);
          }
        } else if (isAck && !isFromDest) {
          // Relay ACK banner
          snprintf(buf, sizeof(buf), "DM Relayed\n(Status Unknown)\n%s\n\nSignal: %s", (nodeName && nodeName[0]) ? nodeName : "unknown",
                   qualityLabel);
        } else {
          if (this->lastSentNode == NODENUM_BROADCAST) {
            snprintf(buf, sizeof(buf), "Message failed to\n#%s", (channelName && channelName[0]) ? channelName : "unknown");
          } else {
...
void CannedMessageModule::loadProtoForModule() {
  if (nodeDB->loadProto(cannedMessagesConfigFile, meshtastic_CannedMessageModuleConfig_size, sizeof(meshtastic_CannedMessageModuleConfig),
                        &meshtastic_CannedMessageModuleConfig_msg, &cannedMessageModuleConfig) != LoadFileResult::LOAD_SUCCESS) {
