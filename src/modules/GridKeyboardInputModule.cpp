...
void GridKeyboardInputModule::handleButtonHeld(uint32_t now, uint32_t duration)
{
...
void GridKeyboardInputModule::handleModeSwitch(int modeIndex)
{
    ...
    } else if (modeIndex == 2) { // Special Characters
        SingleButtonInputManager::instance().setMode(SingleButtonInputManager::MODE_SPECIAL_CHARACTERS);
    } else if (modeIndex == 1) { // Grid Keyboard (current mode)
        // Already in Grid mode, just close menu
        menuOpen = false;
