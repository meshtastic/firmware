...
bool RoutingModule::handleReceivedProtobuf(const meshtastic_MeshPacket &mp,
                                           meshtastic_Routing *r) {
  bool maybePKI =
      mp.which_payload_variant == meshtastic_MeshPacket_encrypted_tag &&
      mp.channel == 0 && !isBroadcast(mp.to);
  ...
  if (mp.which_payload_variant == meshtastic_MeshPacket_encrypted_tag &&
    ...
    if (!maybePKI)
      return false;
    if ((nodeDB->getMeshNode(mp.from) == NULL ||
         !nodeDB->getMeshNode(mp.from)->has_user) &&
        (nodeDB->getMeshNode(mp.to) == NULL ||
         !nodeDB->getMeshNode(mp.to)->has_user))
      return false;
  } else if (owner.is_licensed && nodeDB->getLicenseStatus(mp.from) ==
  ...
  // Note: Phone/API delivery is now handled by PhoneAPIModule
...
void RoutingModule::sendAckNak(meshtastic_Routing_Error err, NodeNum to,
                               PacketId idFrom, ChannelIndex chIndex,
                               uint8_t hopLimit, bool ackWantsAck) {
  auto p = allocAckNak(err, to, idFrom, chIndex, hopLimit);

  // Allow the caller to set want_ack on this ACK packet if it's important that
  // the ACK be delivered reliably
  p->want_ack = ackWantsAck;

  router->sendLocal(p); // we sometimes send directly to the local node
}

uint8_t RoutingModule::getHopLimitForResponse(uint8_t hopStart,
                                              uint8_t hopLimit) {
  if (hopStart != 0) {
    // Hops used by the request. If somebody in between running modified
    // firmware modified it, ignore it
    uint8_t hopsUsed =
        hopStart < hopLimit ? config.lora.hop_limit : hopStart - hopLimit;
...
// hops.
  ...
  return Default::getConfiguredOrDefaultHopLimit(
      config.lora.hop_limit); // Use the default hop limit
}

meshtastic_MeshPacket *RoutingModule::allocAckNak(meshtastic_Routing_Error err,
                                                  NodeNum to, PacketId idFrom,
...
RoutingModule::RoutingModule()
    : ProtobufModule("routing", meshtastic_PortNum_ROUTING_APP,
