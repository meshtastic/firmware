...
static constexpr uint32_t NodeInfoReplySuppressSeconds =
...
bool NodeInfoModule::handleReceivedProtobuf(const meshtastic_MeshPacket &mp,
  ...
  // Store the remote node's ephemeral key if present (for PFS)
  if (ephemeralKeyMgr && p.ephemeral_pubkey.size == 32) {
    ephemeralKeyMgr->setRemoteKey(getFrom(&mp), p.ephemeral_pubkey.bytes,
                                  p.ephemeral_key_id,
                                  p.ephemeral_key_timestamp);
    LOG_DEBUG("Stored ephemeral key from node %08x, keyId=%u", getFrom(&mp),
              p.ephemeral_key_id);
  }
#endif
  ...
  if (hasChanged && !wasBroadcast && !isToUs(&mp)) {
    auto packetCopy = packetPool.allocCopy(
        mp); // Keep a copy of the packet for later analysis

    // Re-encode the user protobuf, as we have stripped out the user.id
    packetCopy->decoded.payload.size = pb_encode_to_bytes(
        packetCopy->decoded.payload.bytes,
        sizeof(packetCopy->decoded.payload.bytes), &meshtastic_User_msg, &p);
...
void NodeInfoModule::alterReceivedProtobuf(meshtastic_MeshPacket &mp,
                                           meshtastic_User *p) {
  // Coerce user.id to be derived from the node number
  snprintf(p->id, sizeof(p->id), "!%08x", getFrom(&mp));

  // Re-encode the altered protobuf back into the packet
  mp.decoded.payload.size = pb_encode_to_bytes(mp.decoded.payload.bytes,
                                               sizeof(mp.decoded.payload.bytes),
                                               &meshtastic_User_msg, p);
}

void NodeInfoModule::sendOurNodeInfo(NodeNum dest, bool wantReplies,
                                     uint8_t channel, bool _shorterTimeout) {
  // cancel any not yet sent (now stale) position packets
  if (prevPacketId) // if we wrap around to zero, we'll simply fail to cancel in
                    // that rare case (no big deal)
...
meshtastic_MeshPacket *NodeInfoModule::allocReply() {
  ...
  // If we sent our NodeInfo less than 5 min. ago, don't send it again as it may
  // be still underway.
  if (!shorterTimeout && lastSentToMesh &&
      Throttle::isWithinTimespanMs(lastSentToMesh, 5 * 60 * 1000)) {
    LOG_DEBUG("Skip send NodeInfo since we sent it <5min ago");
    ignoreRequest = true; // Mark it as ignored for MeshModule
    return NULL;
  } else if (shorterTimeout && lastSentToMesh &&
             Throttle::isWithinTimespanMs(lastSentToMesh, 60 * 1000)) {
...
#if !defined(MESHTASTIC_EXCLUDE_PKI)
    // Add ephemeral key data for Perfect Forward Secrecy
    if (ephemeralKeyMgr && ephemeralKeyMgr->isInitialized()) {
      // Check if we need to rotate before broadcasting
      if (ephemeralKeyMgr->shouldRotate()) {
        ephemeralKeyMgr->rotateKey();
      }

      // Copy ephemeral public key
      const uint8_t *localPubkey = ephemeralKeyMgr->getPublicKey();
      if (localPubkey) {
        memcpy(u.ephemeral_pubkey.bytes, localPubkey, 32);
        u.ephemeral_pubkey.size = 32;
      }

      // Set key ID and timestamp
      u.ephemeral_key_id = ephemeralKeyMgr->getKeyId();
      u.ephemeral_key_timestamp = ephemeralKeyMgr->getKeyTimestamp();
    }
#endif

    // FIXME: Clear the user.id field since it should be derived from node
    // number on the receiving end u.id[0] = '\0';
...
void NodeInfoModule::pruneLastNodeInfoCache() {
  ...
  while (!lastNodeInfoSeen.empty() && lastNodeInfoSeen.size() > maxEntries) {
    auto oldestIt =
        std::min_element(lastNodeInfoSeen.begin(), lastNodeInfoSeen.end(),
...
NodeInfoModule::NodeInfoModule()
    : ProtobufModule("nodeinfo", meshtastic_PortNum_NODEINFO_APP,
                     &meshtastic_User_msg),
      concurrency::OSThread("NodeInfo") {
  isPromiscuous = true; // We always want to update our nodedb, even if we are
                        // sniffing on others

  setIntervalFromNow(
      setStartDelay()); // Send our initial owner announcement 30 seconds
...
int32_t NodeInfoModule::runOnce() {
  ...
  if (airTime->isTxAllowedAirUtil() &&
    ...
    sendOurNodeInfo(NODENUM_BROADCAST,
                    requestReplies); // Send our info (don't request replies)
  }
  return Default::getConfiguredOrDefaultMs(
