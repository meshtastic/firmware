    ...
    if (moduleConfig.serial.override_console_serial_port || (moduleConfig.serial.rxd && moduleConfig.serial.txd))
    {
        ...
        else
        {
            ...
            else if (moduleConfig.serial.mode == meshtastic_ModuleConfig_SerialConfig_Serial_Mode_NMEA)
            {
                ...
                {
                    lastNmeaTime = millis();
                    printGGA(outbuf, sizeof(outbuf), nodeDB.getNodeInfo(myNodeInfo.my_node_num)->position);
        ...
        else
        {
                     ...
                     moduleConfig.serial.mode == meshtastic_ModuleConfig_SerialConfig_Serial_Mode_CALTOPO)
            {
                ...
                if (mp.which_payload_variant == meshtastic_MeshPacket_decoded_tag && mp.decoded.portnum == ourPortNum)
                {
                    ...
                    printWPL(outbuf, sizeof(outbuf), *decoded, nodeDB.getNodeInfo(getFrom(&mp))->user.long_name,
                             moduleConfig.serial.mode == meshtastic_ModuleConfig_SerialConfig_Serial_Mode_CALTOPO);
                    serialPrint->printf("%s", outbuf);
                }
            }
