...
*/
void NeighborInfoModule::printNeighborInfo(const char *header,
                                           const meshtastic_NeighborInfo *np) {
  LOG_DEBUG(
      "%s NEIGHBORINFO PACKET from Node 0x%x to Node 0x%x (last sent by 0x%x)",
      header, np->node_id, nodeDB->getNodeNum(), np->last_sent_by_id);
  LOG_DEBUG("Packet contains %d neighbors", np->neighbors_count);
  for (int i = 0; i < np->neighbors_count; i++) {
    LOG_DEBUG("Neighbor %d: node_id=0x%x, snr=%.2f", i,
              np->neighbors[i].node_id, np->neighbors[i].snr);
  }
}
...
void NeighborInfoModule::printNodeDBNeighbors() {
  ...
  for (size_t i = 0; i < neighbors.size(); i++) {
    LOG_DEBUG("Node %d: node_id=0x%x, snr=%.2f", i, neighbors[i].node_id,
              neighbors[i].snr);
  }
}
...
NeighborInfoModule::NeighborInfoModule()
    : ProtobufModule("neighborinfo", meshtastic_PortNum_NEIGHBORINFO_APP,
  ...
  if (moduleConfig.neighbor_info.enabled) {
    ...
    setIntervalFromNow(Default::getConfiguredOrDefaultMs(
...
*/
uint32_t
NeighborInfoModule::collectNeighborInfo(meshtastic_NeighborInfo *neighborInfo) {
  NodeNum my_node_id = nodeDB->getNodeNum();
  neighborInfo->node_id = my_node_id;
  neighborInfo->last_sent_by_id = my_node_id;
  neighborInfo->node_broadcast_interval_secs = Default::getConfiguredOrDefault(
      moduleConfig.neighbor_info.update_interval,
      default_telemetry_broadcast_interval_secs);
  ...
  for (auto nbr : neighbors) {
    if ((neighborInfo->neighbors_count < MAX_NUM_NEIGHBORS) &&
        (nbr.node_id != my_node_id)) {
      neighborInfo->neighbors[neighborInfo->neighbors_count].node_id =
          nbr.node_id;
...
void NeighborInfoModule::cleanUpNeighbors() {
  ...
  for (auto it = neighbors.rbegin(); it != neighbors.rend();) {
    ...
    // seconds since 1970
    if ((now - it->last_rx_time > it->node_broadcast_interval_secs * 2) &&
        (it->node_id != my_node_id)) {
      LOG_DEBUG("Remove neighbor with node ID 0x%x", it->node_id);
      it = std::vector<meshtastic_Neighbor>::reverse_iterator(neighbors.erase(
          std::next(it).base())); // Erase the element and update the iterator
    } else {
      ++it;
    }
  }
...
int32_t NeighborInfoModule::runOnce() {
  if (moduleConfig.neighbor_info.transmit_over_lora &&
      (!channels.isDefaultChannel(channels.getPrimaryIndex()) ||
...
*/
bool NeighborInfoModule::handleReceivedProtobuf(const meshtastic_MeshPacket &mp,
...
*/
void NeighborInfoModule::alterReceivedProtobuf(meshtastic_MeshPacket &p,
                                               meshtastic_NeighborInfo *n) {
  ...
  p.decoded.payload.size = pb_encode_to_bytes(p.decoded.payload.bytes,
                                              ...
                                              &meshtastic_NeighborInfo_msg, n);
}

void NeighborInfoModule::resetNeighbors() { neighbors.clear(); }

void NeighborInfoModule::updateNeighbors(const meshtastic_MeshPacket &mp,
                                         const meshtastic_NeighborInfo *np) {
  ...
  if (mp.which_payload_variant == meshtastic_MeshPacket_decoded_tag &&
      mp.from) {
    getOrCreateNeighbor(mp.from, np->last_sent_by_id,
                        np->node_broadcast_interval_secs, mp.rx_snr);
  }
}

meshtastic_Neighbor *
NeighborInfoModule::getOrCreateNeighbor(NodeNum originalSender, NodeNum n,
  ...
  for (size_t i = 0; i < neighbors.size(); i++) {
    if (neighbors[i].node_id == n) {
      ...
      if (originalSender == n && node_broadcast_interval_secs != 0)
        neighbors[i].node_broadcast_interval_secs =
  ...
  if (originalSender == n && node_broadcast_interval_secs != 0)
    new_nbr.node_broadcast_interval_secs = node_broadcast_interval_secs;
       ...
       // know it
    new_nbr.node_broadcast_interval_secs =
        moduleConfig.neighbor_info.update_interval;
