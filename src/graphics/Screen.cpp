...
float Screen::estimatedHeading(double lat, double lon)
{
    ...
    if (d < 10) {
        ...
        Screen::Screen(ScanI2C::DeviceAddress address, meshtastic_Config_DisplayConfig_OledType screenType,
            ...
            : concurrency::OSThread("Screen"), address_found(address), model(screenType), geometry(geometry), cmdQueue(32)
        {
            ...
            dispdev = new ST7789Spi(&SPI1, ST7789_RESET, ST7789_RS, ST7789_NSS, GEOMETRY_RAWMODE, TFT_WIDTH, TFT_HEIGHT,
                                    ST7789_SDA, ST7789_MISO, ST7789_SCK);
#else
            dispdev = new ST7789Spi(&SPI1, ST7789_RESET, ST7789_RS, ST7789_NSS, GEOMETRY_RAWMODE, TFT_WIDTH, TFT_HEIGHT);
#endif
            ...
            dispdev = new ST7796Spi(&SPI1, ST7796_RESET, ST7796_RS, ST7796_NSS, GEOMETRY_RAWMODE, TFT_WIDTH, TFT_HEIGHT,
                                    ST7796_SDA, ST7796_MISO, ST7796_SCK, TFT_SPI_FREQUENCY);
#else
            dispdev = new ST7796Spi(&SPI1, ST7796_RESET, ST7796_RS, ST7796_NSS, GEOMETRY_RAWMODE, TFT_WIDTH, TFT_HEIGHT);
#endif
        ...
        void Screen::setup()
        {
            ...
            if (config.display.displaymode != meshtastic_Config_DisplayConfig_DisplayMode_COLOR) {
                if (portduino_config.touchscreenModule) {
                    touchScreenImpl1 = new TouchScreenImpl1(dispdev->getWidth(), dispdev->getHeight(),
        ...
        int32_t Screen::runOnce()
        {
            ...
            for (;;) {
                ...
                case Cmd::START_ALERT_FRAME: {
                    showingBootScreen =
                        false; // this should avoid the edge case where an alert triggers before the boot screen goes away
...
// Show detailed node views only on E-Ink builds
        ...
        void Screen::logFrameChange(const char *reason, uint8_t targetIdx)
        {
            ...
            if (targetIdx == p.home)
                ...
                name = "wifi";
            else if (p.firstFavorite != 255 && p.lastFavorite != 255 && targetIdx >= p.firstFavorite &&
                     targetIdx <= p.lastFavorite)
                name = "favorite";
            LOG_INFO("Screen: frame %u/%u name=%s reason=%s", (unsigned)targetIdx, (unsigned)framesetInfo.frameCount, name,
                     reason);
        }
#endif
...
#define SCREEN_TRANSITION_FRAMERATE 30 // fps
        ...
        int Screen::handleInputEvent(const InputEvent *event)
        {
            ...
            if (framesetInfo.positions.favoritesMap != 255 &&
                ui->getUiState()->currentFrame == framesetInfo.positions.favoritesMap) {
                if (event->inputEvent == INPUT_BROKER_UP) {
            ...
            if (showingNormalScreen) {
                ...
                if (!inputIntercepted) {
                    ...
                    const bool isTextMessageFrame = (framesetInfo.positions.textMessage != 255 &&
                                                     ...
                                                     messageStore.hasVisibleMessages());
