...
- Keep InkHUD code within `/src/graphics/niche`.
- Reusable E-Ink driver, panel, backlight, and BaseUI-adapter code lives in `/src/graphics/eink` so other UIs (including the default BaseUI) can use the same hardware layer without pulling InkHUD.
...
- Is self-contained
- Makes use of shared E-Ink hardware resources (drivers, panels, backlight, BaseUI adapter) in `/src/graphics/eink`, plus InkHUD-specific resources (input helpers, fonts, applets, storage) in `/src/graphics/niche`.
...
For more information, see the documentation in `src/graphics/eink/Drivers`
...
See `src/graphics/niche/Applets/Examples` for example code.
...
_`src/graphics/niche/InkHUD.h`_
...
_`src/graphics/niche/Persistence.h`_
...
_`src/graphics/niche/WindowManager.h`_
...
_`src/graphics/niche/Renderer.h`_
...
_`src/graphics/niche/DisplayHealth.h`_
