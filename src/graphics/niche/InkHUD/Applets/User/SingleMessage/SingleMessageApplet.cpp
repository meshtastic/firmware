...
void InkHUD::SingleMessageApplet::render()
{
    ...
    if (!settings.lastMessage.nodeNum) {
    ...
    // RX Time
    // - if valid
    std::string timeString = getTimeString(settings.lastMessage.timestamp);
    ...
    // - node id
    meshtastic_NodeInfoLite *sender = nodeDB->getMeshNode(settings.lastMessage.nodeNum);
    if (sender && sender->has_user) {
        ...
        header += hexifyNodeNum(settings.lastMessage.nodeNum);
    ...
    setFont(fontLarge);
    uint32_t textHeight = getWrappedTextHeight(0, width(), settings.lastMessage.text);
    ...
    // Print text
    printWrapped(0, textTop, width(), settings.lastMessage.text);
}
