...
InkHUD::Tile *InkHUD::Applet::getTile() { return assignedTile; }
...
bool InkHUD::Applet::wantsToRender() { return wantRender; }
...
// Used by WindowManager::update
bool InkHUD::Applet::wantsToAutoshow() { return wantAutoshow; }

// Which technique would this applet prefer that the display use to change the image?
// Used by WindowManager::update
Drivers::EInk::UpdateTypes InkHUD::Applet::wantsUpdateType() { return wantUpdateType; }
...
void InkHUD::Applet::requestAutoshow() { wantAutoshow = true; }
...
bool InkHUD::Applet::isActive() { return active; }
...
bool InkHUD::Applet::isForeground() { return foreground; }
...
void InkHUD::Applet::resetCrop() { setCrop(0, 0, width(), height()); }

// Convert relative width to absolute width, in px
// X(0) is 0
// X(0.5) is width() / 2
// X(1) is width()
uint16_t InkHUD::Applet::X(float f) { return width() * f; }

// Convert relative hight to absolute height, in px
// Y(0) is 0
// Y(0.5) is height() / 2
// Y(1) is height()
uint16_t InkHUD::Applet::Y(float f) { return height() * f; }
...
// This is AppletFont type, which is a wrapper for AdafruitGFX font, with some precalculated dimension data
InkHUD::AppletFont InkHUD::Applet::getFont() { return currentFont; }

// Parse any text which might have "special characters"
// Re-encodes UTF-8 characters to match our 8-bit encoded fonts
std::string InkHUD::Applet::parse(std::string text) { return getFont().decodeUTF8(text); }
...
// Gets rendered width of a string
// Wrapper for getTextBounds
uint16_t InkHUD::Applet::getTextWidth(std::string text) { return getTextWidth(text.c_str()); }
...
std::string InkHUD::Applet::getTimeString() { return getTimeString(getValidTime(RTCQuality::RTCQualityDevice, true)); }
