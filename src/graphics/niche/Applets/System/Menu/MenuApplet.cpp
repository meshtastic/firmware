...
static constexpr uint8_t MENU_TIMEOUT_SEC =
    60; // How many seconds before menu auto-closes

// Options for the "Recents" menu
// These are offered to users as possible values for
...
static constexpr DisplayTimeoutOption DISPLAY_TIMEOUT_OPTIONS[] = {
    {0, "Forever"},    {30, "30 secs"},     {60, "1 min"},
    {5 * 60, "5 min"}, {15 * 60, "15 min"}, {30 * 60, "30 min"},
    {60 * 60, "1 hr"},
};
...
static constexpr PositionPrecisionOption POSITION_PRECISION_OPTIONS[] = {
    {32, "Precise", "Precise"}, {19, "50 m", "150 ft"},
    {18, "90 m", "300 ft"},     {17, "200 m", "600 ft"},
    {16, "350 m", "0.2 mi"},    {15, "700 m", "0.5 mi"},
    {14, "1.5 km", "0.9 mi"},   {13, "2.9 km", "1.8 mi"},
    {12, "5.8 km", "3.6 mi"},   {11, "12 km", "7.3 mi"},
    {10, "23 km", "15 mi"},
};

static const char *getDisplayTimeoutLabel(uint32_t timeoutSeconds) {
  constexpr uint8_t optionCount =
...
static bool
supportsFreeTextKeyboard(const InkHUD::InkHUD *inkhud,
                         const InkHUD::Persistence::Settings *settings) {
  return !inkhud->twoWayRocker &&
         (settings->joystick.enabled || inkhud->hasTouchEnabledProvider());
}
...
static uint16_t getMenuItemHeightPx(const InkHUD::InkHUD *inkhud) {
  ...
  const uint16_t lineH = touchFriendly ? InkHUD::Applet::fontMedium.lineHeight()
                                       : InkHUD::Applet::fontSmall.lineHeight();
...
InkHUD::MenuApplet::MenuApplet() : concurrency::OSThread("MenuApplet") {
  ...
  // - handles setting / getting of canned messages via apps (Client API Admin
  // Messages)
  cm.store = CannedMessageStore::getInstance();
}

void InkHUD::MenuApplet::onForeground() {
  // We do need this before we render, but we can optimize by just calculating
  // it once now
...
void InkHUD::MenuApplet::onBackground() {
  ...
  // If this item is deselected we will turn backlight off again, now that menu
  // is closing
  if (settings->optionalMenuItems.backlight) {
#if !defined(T5_S3_EPAPER_PRO)
    assert(backlight);
  ...
  Tile *t = getTile();
  t->assignApplet(
      borrowedTileOwner); // Break our link with the tile, (and relink it with
                          // real owner, if it had one)
  borrowedTileOwner = nullptr;

  // Need to force an update, as a polite request wouldn't be honored, seeing
  // how we are now in the background We're only updating here to upgrade from
  // UNSPECIFIED to FAST, to ensure responsiveness when exiting menu
  inkhud->forceUpdate(EInk::UpdateTypes::FAST);
}
...
// The values shown on the root menu are only a snapshot: they are not
// re-rendered while the menu remains open. By exiting the menu, we prevent
// users mistakenly believing that the data will update.
int32_t InkHUD::MenuApplet::runOnce() {
  // runOnce's interval is pushed back when a button is pressed
  // If we do actually run, it means no button input occurred within
  // MENU_TIMEOUT_SEC, so we close the menu.
  showPage(EXIT);
...
// Storage for the dynamically-built region preset list — populated in
// showPage(NODE_CONFIG_PRESET)
static constexpr uint8_t MAX_REGION_PRESETS = 16;
static meshtastic_Config_LoRaConfig_ModemPreset
    regionPresets[MAX_REGION_PRESETS];
...
static void applyLoRaRegion(meshtastic_Config_LoRaConfig_RegionCode region) {
  ...
  if (strncmp(moduleConfig.mqtt.root, default_mqtt_root,
              strlen(default_mqtt_root)) == 0) {
...
void InkHUD::MenuApplet::execute(MenuItem item) {
  ...
  // Also handles exit
  case NO_ACTION:
    if (currentPage == MenuPage::NODE_CONFIG_CHANNELS &&
        item.nextPage == MenuPage::NODE_CONFIG_CHANNEL_DETAIL) {
  ...
  case SEND_PING:
    ...
    // Force the next refresh to use FULL, to protect the display, as some users
    // will probably spam this button
  ...
  case STORE_CANNEDMESSAGE_SELECTION: {
    const uint8_t prefixItems =
        supportsFreeTextKeyboard(inkhud, settings) ? 2 : 1;
  ...
  case SEND_CANNEDMESSAGE:
    ...
    sendText(cm.selectedRecipientItem->dest,
             cm.selectedRecipientItem->channelIndex,
             cm.selectedMessageItem->rawText.c_str());
    inkhud->forceUpdate(
        Drivers::EInk::UpdateTypes::FULL); // Next refresh should be FULL. Lots
                                           // of button pressing to get here
    break;
  ...
  case LAYOUT:
    ...
    if (settings->userTiles.count >
        settings->userTiles.maxCount) // Loop around if tile count now too high
  ...
  case TOGGLE_AUTOSHOW_APPLET:
    // Toggle settings.userApplets.autoshow[] value, via MenuItem::checkState
    // pointer set in populateAutoshowPage()
    if (item.checkState) {
      *item.checkState = !(*item.checkState);
    }
    break;
  ...
  case SET_RECENTS: {
    ...
    constexpr uint8_t optionCount =
        sizeof(RECENTS_OPTIONS_MINUTES) / sizeof(RECENTS_OPTIONS_MINUTES[0]);
  ...
  case TOGGLE_GPS:
    ...
    } else if (config.position.gps_mode ==
               meshtastic_Config_PositionConfig_GpsMode_ENABLED) {
      config.position.gps_mode =
          meshtastic_Config_PositionConfig_GpsMode_DISABLED;
    } else {
  ...
  case CALIBRATE_ADC: {
    ...
    if (measuredV < 3.0f || measuredV > 4.5f) {
      LOG_WARN("ADC calibration aborted, unreasonable voltage: %.2fV",
               measuredV);
      break;
    }
    ...
    LOG_INFO("ADC calibrated: measured=%.3fV base=%.4f new=%.4f", measuredV,
             baseMult, newMult);
  ...
  case SET_CHANNEL_PRECISION: {
    ...
    constexpr uint8_t optionCount = sizeof(POSITION_PRECISION_OPTIONS) /
                                    sizeof(POSITION_PRECISION_OPTIONS[0]);

    if (index < optionCount) {
      ch.settings.module_settings.position_precision =
          POSITION_PRECISION_OPTIONS[index].value;
    }
...
void InkHUD::MenuApplet::showPage(MenuPage page) {
  ...
  case ROOT:
    ...
    if (settings->optionalMenuItems.nextTile && settings->userTiles.count > 1)
      items.push_back(
          MenuItem("Next Tile", MenuAction::NEXT_TILE,
                   MenuPage::ROOT)); // Only if multiple applets shown
  ...
  case OPTIONS:
...
#endif
      items.push_back(MenuItem("Keep Backlight On",
                               MenuAction::TOGGLE_BACKLIGHT, MenuPage::OPTIONS,
                               &keepBacklightOn));
    }
    ...
    if (settings->userTiles.maxCount > 1)
      items.push_back(
          MenuItem("Layout", MenuAction::LAYOUT, MenuPage::OPTIONS));
    items.push_back(MenuItem("Rotate", MenuAction::ROTATE, MenuPage::OPTIONS));
    if (settings->joystick.enabled && !inkhud->twoWayRocker)
      items.push_back(MenuItem("Align Joystick", MenuAction::ALIGN_JOYSTICK,
                               MenuPage::EXIT));
    items.push_back(MenuItem("Notifications", MenuAction::TOGGLE_NOTIFICATIONS,
                             MenuPage::OPTIONS,
                             &settings->optionalFeatures.notifications));
    items.push_back(MenuItem("Battery Icon", MenuAction::TOGGLE_BATTERY_ICON,
                             MenuPage::OPTIONS,
                             &settings->optionalFeatures.batteryIcon));
    invertedColors = (config.display.displaymode ==
                      meshtastic_Config_DisplayConfig_DisplayMode_INVERTED);
    items.push_back(MenuItem("Invert Color", MenuAction::TOGGLE_INVERT_COLOR,
                             MenuPage::OPTIONS, &invertedColors));
    items.push_back(MenuItem("Exit", MenuPage::EXIT));
    break;
  ...
  case NODE_CONFIG:
    ...
    items.push_back(
        MenuItem("Reset NodeDB", MenuPage::NODE_CONFIG_ADMIN_RESET));
  ...
  case NODE_CONFIG_DEVICE: {
    ...
    items.push_back(MenuItem(nodeConfigLabels.back().c_str(),
                             MenuAction::NO_ACTION,
                             MenuPage::NODE_CONFIG_DEVICE_ROLE));
    ...
    items.push_back(MenuItem(nodeConfigLabels.back().c_str(),
                             MenuAction::NO_ACTION, MenuPage::TIMEZONE));
  ...
  case NODE_CONFIG_POSITION: {
    ...
    if (mode == meshtastic_Config_PositionConfig_GpsMode_NOT_PRESENT) {
      items.push_back(MenuItem("GPS None", MenuAction::NO_ACTION,
                               MenuPage::NODE_CONFIG_POSITION));
    } else {
      gpsEnabled = (mode == meshtastic_Config_PositionConfig_GpsMode_ENABLED);
      items.push_back(MenuItem("GPS", MenuAction::TOGGLE_GPS,
                               MenuPage::NODE_CONFIG_POSITION, &gpsEnabled));
    }
#endif
    items.push_back(MenuItem("Exit", MenuPage::EXIT));
    break;
  ...
  case NODE_CONFIG_POWER: {
    ...
    items.push_back(MenuItem("Back", previousPage));
#if defined(ARCH_ESP32)
    items.push_back(MenuItem("Powersave", MenuAction::TOGGLE_POWER_SAVE,
                             MenuPage::EXIT, &config.power.is_power_saving));
#endif
    // ADC Multiplier
    ...
    if (effectiveMult > 0.0f) {
      ...
      items.push_back(MenuItem(nodeConfigLabels.back().c_str(),
                               MenuAction::NO_ACTION,
                               MenuPage::NODE_CONFIG_POWER_ADC_CAL));
    }
  ...
  case NODE_CONFIG_POWER_ADC_CAL: {
    ...
    // Action
    items.push_back(MenuItem("Calibrate ADC", MenuAction::CALIBRATE_ADC,
  ...
  case NODE_CONFIG_NETWORK: {
    ...
    const char *wifiLabel =
        config.network.wifi_enabled ? "WiFi: On" : "WiFi: Off";

    items.push_back(
        MenuItem(wifiLabel, MenuAction::TOGGLE_WIFI, MenuPage::EXIT));
  ...
  case NODE_CONFIG_DISPLAY: {
    ...
    items.push_back(MenuItem("12-Hour Clock", MenuAction::TOGGLE_12H_CLOCK,
                             MenuPage::NODE_CONFIG_DISPLAY,
                             &config.display.use_12h_clock));

    nodeConfigLabels.emplace_back(
        "Screen Timeout: " +
        std::string(getDisplayTimeoutLabel(config.display.screen_on_secs)));
    items.push_back(MenuItem(nodeConfigLabels.back().c_str(),
                             MenuAction::NO_ACTION,
                             MenuPage::NODE_CONFIG_DISPLAY_TIMEOUT));
    ...
    items.push_back(MenuItem(unitsLabel, MenuAction::TOGGLE_DISPLAY_UNITS,
                             MenuPage::NODE_CONFIG_DISPLAY));
  ...
  case NODE_CONFIG_BLUETOOTH: {
    ...
    const char *btLabel =
        config.bluetooth.enabled ? "Bluetooth: On" : "Bluetooth: Off";
    items.push_back(
        MenuItem(btLabel, MenuAction::TOGGLE_BLUETOOTH, MenuPage::EXIT));

    const char *pairLabel =
        config.bluetooth.fixed_pin ? "Pair Mode: Fixed" : "Pair Mode: Random";
    items.push_back(MenuItem(pairLabel, MenuAction::TOGGLE_BLUETOOTH_PAIR_MODE,
                             MenuPage::NODE_CONFIG_BLUETOOTH));
  ...
  case NODE_CONFIG_LORA: {
    ...
    items.push_back(MenuItem(nodeConfigLabels.back().c_str(),
                             MenuAction::NO_ACTION, MenuPage::REGION));

    const char *preset = DisplayFormatters::getModemPresetDisplayName(
        config.lora.modem_preset, false, config.lora.use_preset);
    nodeConfigLabels.emplace_back("Preset: " + std::string(preset));
    items.push_back(MenuItem(nodeConfigLabels.back().c_str(),
                             MenuAction::NO_ACTION,
                             MenuPage::NODE_CONFIG_PRESET));
  ...
  case NODE_CONFIG_CHANNEL_DETAIL: {
    ...
    // Name (read-only)
    const char *name =
        strlen(ch.settings.name) > 0 ? ch.settings.name : "Unnamed";
    ...
    // Uplink
    items.push_back(MenuItem("Uplink", MenuAction::TOGGLE_CHANNEL_UPLINK,
                             MenuPage::NODE_CONFIG_CHANNEL_DETAIL,
                             &ch.settings.uplink_enabled));

    items.push_back(MenuItem("Downlink", MenuAction::TOGGLE_CHANNEL_DOWNLINK,
                             MenuPage::NODE_CONFIG_CHANNEL_DETAIL,
                             &ch.settings.downlink_enabled));

    // Position
    channelPositionEnabled = ch.settings.has_module_settings &&
                             ch.settings.module_settings.position_precision > 0;

    items.push_back(MenuItem("Position", MenuAction::TOGGLE_CHANNEL_POSITION,
                             MenuPage::NODE_CONFIG_CHANNEL_DETAIL,
    ...
    if (channelPositionEnabled) {
      ...
      items.push_back(MenuItem(nodeConfigLabels.back().c_str(),
                               MenuAction::NO_ACTION,
                               MenuPage::NODE_CONFIG_CHANNEL_PRECISION));
    }
  ...
  case NODE_CONFIG_CHANNEL_PRECISION: {
    ...
    if (!ch.settings.has_module_settings ||
        ch.settings.module_settings.position_precision == 0) {
      items.push_back(
          MenuItem("Position is Off", MenuPage::NODE_CONFIG_CHANNEL_DETAIL));
      break;
    }
    constexpr uint8_t optionCount = sizeof(POSITION_PRECISION_OPTIONS) /
                                    sizeof(POSITION_PRECISION_OPTIONS[0]);
    for (uint8_t i = 0; i < optionCount; i++) {
      ...
      items.push_back(MenuItem(nodeConfigLabels.back().c_str(),
                               MenuAction::SET_CHANNEL_PRECISION,
                               MenuPage::NODE_CONFIG_CHANNEL_DETAIL));
    }
    items.push_back(MenuItem("Exit", MenuPage::EXIT));
    break;
  ...
  case NODE_CONFIG_DEVICE_ROLE: {
    ...
    items.push_back(MenuItem("Back", previousPage));
    items.push_back(
        MenuItem("Client", MenuAction::SET_ROLE_CLIENT, MenuPage::EXIT));
    items.push_back(MenuItem("Client Mute", MenuAction::SET_ROLE_CLIENT_MUTE,
                             MenuPage::EXIT));
    items.push_back(
        MenuItem("Router", MenuAction::SET_ROLE_ROUTER, MenuPage::EXIT));
    items.push_back(
        MenuItem("Repeater", MenuAction::SET_ROLE_REPEATER, MenuPage::EXIT));
    items.push_back(MenuItem("Exit", MenuPage::EXIT));
    break;
  }
  ...
  case TIMEZONE:
    ...
    items.push_back(MenuItem("Back", previousPage));
    items.push_back(
        MenuItem("US/Hawaii", SET_TZ_US_HAWAII, MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(
        MenuItem("US/Alaska", SET_TZ_US_ALASKA, MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("US/Pacific", SET_TZ_US_PACIFIC,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("US/Arizona", SET_TZ_US_ARIZONA,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("US/Mountain", SET_TZ_US_MOUNTAIN,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("US/Central", SET_TZ_US_CENTRAL,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("US/Eastern", SET_TZ_US_EASTERN,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("BR/Brasilia", SET_TZ_BR_BRAZILIA,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("UTC", SET_TZ_UTC, MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("EU/Western", SET_TZ_EU_WESTERN,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("EU/Central", SET_TZ_EU_CENTRAL,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("EU/Eastern", SET_TZ_EU_EASTERN,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("Asia/Kolkata", SET_TZ_ASIA_KOLKATA,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("Asia/Hong Kong", SET_TZ_ASIA_HONG_KONG,
                             MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(
        MenuItem("AU/AWST", SET_TZ_AU_AWST, MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(
        MenuItem("AU/ACST", SET_TZ_AU_ACST, MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(
        MenuItem("AU/AEST", SET_TZ_AU_AEST, MenuPage::NODE_CONFIG_DEVICE));
    items.push_back(MenuItem("Exit", MenuPage::EXIT));
    break;
  ...
  case REGION:
    ...
    items.push_back(
        MenuItem("EU 868", MenuAction::SET_REGION_EU_868, MenuPage::EXIT));
    items.push_back(
        MenuItem("EU 433", MenuAction::SET_REGION_EU_433, MenuPage::EXIT));
    items.push_back(
        MenuItem("EU 866", MenuAction::SET_REGION_EU_866, MenuPage::EXIT));
    items.push_back(MenuItem("EU 868 Narrow", MenuAction::SET_REGION_NARROW_868,
                             MenuPage::EXIT));
    ...
    items.push_back(
        MenuItem("ANZ", MenuAction::SET_REGION_ANZ, MenuPage::EXIT));
    ...
    items.push_back(
        MenuItem("NZ 865", MenuAction::SET_REGION_NZ_865, MenuPage::EXIT));
    items.push_back(MenuItem("TH", MenuAction::SET_REGION_TH, MenuPage::EXIT));
    items.push_back(
        MenuItem("LoRa 2.4", MenuAction::SET_REGION_LORA_24, MenuPage::EXIT));
    items.push_back(
        MenuItem("UA 433", MenuAction::SET_REGION_UA_433, MenuPage::EXIT));
    items.push_back(
        MenuItem("UA 868", MenuAction::SET_REGION_UA_868, MenuPage::EXIT));
    items.push_back(
        MenuItem("MY 433", MenuAction::SET_REGION_MY_433, MenuPage::EXIT));
    items.push_back(
        MenuItem("MY 919", MenuAction::SET_REGION_MY_919, MenuPage::EXIT));
    items.push_back(
        MenuItem("SG 923", MenuAction::SET_REGION_SG_923, MenuPage::EXIT));
    items.push_back(
        MenuItem("PH 433", MenuAction::SET_REGION_PH_433, MenuPage::EXIT));
    items.push_back(
        MenuItem("PH 868", MenuAction::SET_REGION_PH_868, MenuPage::EXIT));
    items.push_back(
        MenuItem("PH 915", MenuAction::SET_REGION_PH_915, MenuPage::EXIT));
    items.push_back(
        MenuItem("ANZ 433", MenuAction::SET_REGION_ANZ_433, MenuPage::EXIT));
    items.push_back(
        MenuItem("KZ 433", MenuAction::SET_REGION_KZ_433, MenuPage::EXIT));
    items.push_back(
        MenuItem("KZ 863", MenuAction::SET_REGION_KZ_863, MenuPage::EXIT));
    items.push_back(
        MenuItem("NP 865", MenuAction::SET_REGION_NP_865, MenuPage::EXIT));
    items.push_back(
        MenuItem("BR 902", MenuAction::SET_REGION_BR_902, MenuPage::EXIT));
    items.push_back(MenuItem("Exit", MenuPage::EXIT));
    break;
  ...
  case NODE_CONFIG_PRESET: {
    ...
    if (myRegion && myRegion->profile) {
      const meshtastic_Config_LoRaConfig_ModemPreset *presets =
          myRegion->getAvailablePresets();
      size_t numPresets = myRegion->getNumPresets();
      for (size_t i = 0;
           i < numPresets && regionPresetCount < MAX_REGION_PRESETS; ++i) {
        regionPresets[regionPresetCount++] = presets[i];
        const char *name = DisplayFormatters::getModemPresetDisplayName(
            presets[i], false, true);
        nodeConfigLabels.emplace_back(name);
        items.push_back(MenuItem(nodeConfigLabels.back().c_str(),
                                 MenuAction::SET_PRESET_FROM_REGION,
                                 MenuPage::EXIT));
      }
    }
    items.push_back(MenuItem("Exit", MenuPage::EXIT));
  ...
  case NODE_CONFIG_ADMIN_RESET:
    ...
    items.push_back(MenuItem("Back", previousPage));
    items.push_back(
        MenuItem("Reset All", MenuAction::RESET_NODEDB_ALL, MenuPage::EXIT));
    items.push_back(MenuItem("Keep Favorites Only",
                             MenuAction::RESET_NODEDB_KEEP_FAVORITES,
                             MenuPage::EXIT));
    items.push_back(MenuItem("Exit", MenuPage::EXIT));
    break;
  ...
  // Exit
  case EXIT:
    sendToBackground(); // Menu applet dismissed, allow normal behavior to
                        // resume
    break;
  ...
  if (page != currentPage || cursor >= items.size()) {
    ...
    // ROOT menu has special handling: unselected at first, to emphasise the
    // system info panel
    if (page == ROOT)
      cursorShown = false;
  }
...
void InkHUD::MenuApplet::onRender(bool full) {
  ...
  if (freeTextMode) {
    drawInputField(0, fontSmall.lineHeight(), X(1.0),
                   Y(1.0) - fontSmall.lineHeight() - 1,
                   cm.freeTextItem.rawText);
    return;
  }
  ...
  int16_t itemT = 0; // Top (y px of current slot). Incremented as we draw.
                     // Adjusted to fit system info panel on ROOT menu.
  ...
  uint16_t &siH =
      systemInfoPanelHeight; // System info - height. Calculated at onForeground
  const uint8_t slotsObscured = ceilf(
      siH / (float)itemH); // How many slots are obscured by system info panel

  // System info - top
  // Remain at 0px, until cursor reaches bottom of screen, then begin to scroll
  // off screen. This is the same behavior we expect from the non-root menus.
  ...
  int16_t siT;
  if (cursor < slotCount - slotsObscured -
                   1) // (Minus 1: comparing zero based index with a count)
    siT = 0;
  else
  ...
  if (currentPage == ROOT) {
    ...
    itemT = max(siT + siH, 0); // Offset the first menu entry, so menu starts
                               // below the system info panel
  }

  // drawSystemInfoPanel() changes font state (clock/info text).
  // Restore the active menu font so ROOT page item text matches other menu
  // pages, including touch-friendly layouts.
  setFont(menuItemFont);
  ...
  // This may be beyond the slot-count, to draw a partially off-screen item
  // below the bottom-most slow This may be less than the slot-count, if we are
  // reaching the end of the menuItems
  uint8_t lastItem =
      min((uint8_t)firstItem + slotCount, (uint8_t)items.size() - 1);
  ...
  for (uint8_t i = firstItem; i <= lastItem; i++) {
    ...
    if (item.isHeader) {
      ...
      drawLine(itemL + X(padding), underlineY, itemR - X(padding), underlineY,
               BLACK);
    } else {
      // Box, if currently selected
      if (cursorShown && i == cursor &&
          (!touchFriendlyLayout || !hideTouchSelectionHighlight)) {
        ...
        if (touchFriendlyLayout) {
          ...
          fillRect(itemL + markerInset, selTop + markerInset, markerW,
                   max(1, selH - (markerInset * 2)), BLACK);
        }
      }
    ...
    if (item.checkState) {
      const uint16_t cbWH =
          menuItemFont.lineHeight();                 // Checkbox: width / height
...
bool InkHUD::MenuApplet::onTouchPoint(uint16_t x, uint16_t y, bool longPress) {
  ...
  // If button-driven selection is active on touch-first layouts, clear it as
  // soon as touch interaction resumes so touch behavior remains
  // direct/tap-first.
  ...
  // Keep geometry in sync with onRender() so touch hit-testing matches what
  // users see.
  const uint16_t itemH = getMenuItemHeightPx(inkhud);
  int16_t itemT = 0;
  uint8_t slotCount = (height() - itemT) / itemH;
  ...
  if (currentPage == ROOT) {
    int16_t siT = 0;
    const int16_t scrollThreshold =
        (int16_t)slotCount - (int16_t)slotsObscured - 1;
  ...
  const uint8_t firstItem =
      (cursor < slotCount) ? 0 : (cursor - (slotCount - 1));
  ...
  // Consume taps on menu whitespace so we don't trigger button-like fallback
  // behavior.
  return true;
}
...
void InkHUD::MenuApplet::onButtonShortPress() {
  if (!freeTextMode) {
    ...
    // Touch-first nodes keep user-button short-press as "advance selection" in
    // menus. Any button-driven navigation should restore visible highlight.
...
void InkHUD::MenuApplet::onButtonLongPress() {
  if (!freeTextMode) {
    ...
    if (cursorShown)
      ...
      showPage(MenuPage::EXIT); // Special case: Peek at root-menu; longpress
                                // again to close

    // If we didn't already request a specialized update, when handling a menu
    // action, then perform the usual fast update. FAST keeps things responsive:
    // important because we're dealing with user input
    if (!wantsToRender())
      requestUpdate(Drivers::EInk::UpdateTypes::FAST);
  }
}
...
void InkHUD::MenuApplet::onNavUp() {
  if (!freeTextMode) {
    ...
    // Keep cursor movement for scroll math, but selection box is hidden in
    // onRender().
    if (useTouchFriendlyMenuLayout(inkhud)) {
      hideTouchSelectionHighlight = true;
      if (!cursorShown) {
...
// Dynamically create MenuItem entries for activating / deactivating Applets,
// for the "Applet Selection" submenu
void InkHUD::MenuApplet::populateAppletPage() {
  ...
  for (uint8_t i = 0; i < inkhud->userApplets.size(); i++) {
    ...
    items.push_back(
        MenuItem(name, MenuAction::TOGGLE_APPLET, MenuPage::APPLETS, isActive));
  }
}

// Dynamically create MenuItem entries for selecting which applets will
// automatically come to foreground when they have new data We only populate
// this menu page with applets which are actually active We use the
// MenuItem::checkState pointer to toggle the setting in MenuApplet::execute.
// Bit of a hack, but convenient.
void InkHUD::MenuApplet::populateAutoshowPage() {
  ...
  for (uint8_t i = 0; i < inkhud->userApplets.size(); i++) {
    ...
    if (settings->userApplets.active[i]) {
      ...
      items.push_back(MenuItem(name, MenuAction::TOGGLE_AUTOSHOW_APPLET,
                               MenuPage::AUTOSHOW, isActive));
    }
  }
}
...
void InkHUD::MenuApplet::populateRecentsPage() {
  ...
  for (uint8_t i = 0; i < optionCount; i++) {
    ...
    recentsSelected[i] =
        (settings->recentlyActiveSeconds == RECENTS_OPTIONS_MINUTES[i] * 60);
    items.push_back(MenuItem(label.c_str(), MenuAction::SET_RECENTS,
                             MenuPage::OPTIONS, &recentsSelected[i]));
  }
}
...
void InkHUD::MenuApplet::populateDisplayTimeoutPage() {
  ...
  for (uint8_t i = 0; i < optionCount; i++) {
    displayTimeoutSelected[i] =
        (config.display.screen_on_secs == DISPLAY_TIMEOUT_OPTIONS[i].seconds);
    nodeConfigLabels.emplace_back(DISPLAY_TIMEOUT_OPTIONS[i].label);
    items.push_back(MenuItem(
        nodeConfigLabels.back().c_str(), MenuAction::SET_DISPLAY_TIMEOUT,
        MenuPage::NODE_CONFIG_DISPLAY, &displayTimeoutSelected[i]));
  }
}
...
void InkHUD::MenuApplet::populateSendPage() {
  ...
  if (supportsFreeTextKeyboard(inkhud, settings))
    items.push_back(
        MenuItem("Free Text", MenuAction::FREE_TEXT, MenuPage::SEND));
  ...
  for (uint8_t i = 0; i < count; i++) {
    ...
    items.push_back(MenuItem(itemText,
                             MenuAction::STORE_CANNEDMESSAGE_SELECTION,
                             MenuPage::CANNEDMESSAGE_RECIPIENT));
  }
...
void InkHUD::MenuApplet::populateRecipientPage() {
  ...
  if (favoriteCount < 20) {
    for (uint32_t i = 0; i < nodeCount; i++) {
      ...
      r.dest = node->num;
      r.channelIndex = nodeDB->getMeshNodeChannel(
          node->num); // Channel index only relevant if encrypted DM not
                      // possible(?)
      ...
      if (nodeInfoLiteHasUser(node))
        ...
        r.label +=
            hexifyNodeNum(node->num); // Unsure if it's possible to favorite a
                                      // node without NodeInfo?
...
void InkHUD::MenuApplet::drawInputField(uint16_t left, uint16_t top,
  ...
  // If the text is so long that it goes outside of the input box, the text is
  // actually rendered off screen.
  uint32_t textHeight = getWrappedTextHeight(0, width - 5, text);
  if (!text.empty()) {
    uint16_t textPadding =
        X(1.0) > Y(1.0) ? wrapMaxH - textHeight : wrapMaxH - textHeight + 1;
  ...
  uint16_t textCursorY = text.empty()
                             ? fontSmall.lineHeight() + 2
                             : getCursorY() - fontSmall.lineHeight() + 3;
  ...
  // A white rectangle clears the top part of the screen for any text that's
  // printed beyond the input box
  fillRect(0, 0, X(1.0), top, WHITE);

  // Draw character limit
  std::string ftlen =
      std::to_string(text.length()) + "/" + to_string(menuTextLimit);
...
// which we'd prefer not to have displayed in a normal applet, as they update
// too frequently.
void InkHUD::MenuApplet::drawSystemInfoPanel(int16_t left, int16_t top,
                                             uint16_t width,
                                             uint16_t *renderedHeight) {
  // Reset the height
  ...
  height += fontSmall.lineHeight() *
            1.1; // Slightly increased spacing (between bottom line and divider)

  // Position of divider between the info panel and the menu entries
  const int16_t divY = top + height;
  height += fontSmall.lineHeight() *
            0.2; // Padding *below* the divider. (Above first menu item)
  ...
  */
  ...
  for (int16_t x = 0; x < width;
       x += 2) // Divider, centered in the padding between first system panel
               // and first item
...
// This is inefficient, as we do actually have to render the panel to determine
// the height It solves a catch-22 situation, where slotCount needs to know
// panel height, and panel height needs to know slotCount
...
void InkHUD::MenuApplet::sendText(NodeNum dest, ChannelIndex channel,
                                  const char *message) {
  ...
  if (moduleConfig.canned_message.send_bell &&
    ...
    p->decoded.payload.bytes[p->decoded.payload.size + 1] =
        '\0'; // Append Null Terminator
    p->decoded.payload.size++;
  }

  LOG_INFO("Send message id=%d, dest=%x, msg=%.*s", p->id, p->to,
           p->decoded.payload.size, p->decoded.payload.bytes);
