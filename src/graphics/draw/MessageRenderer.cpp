...
ThreadMode getThreadMode() { return currentMode; }
...
const std::vector<int> &getSeenChannels() { return seenChannels; }
const std::vector<uint32_t> &getSeenPeers() { return seenPeers; }
...
void drawTextMessageFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) {
  ...
  for (auto it = filtered.rbegin(); it != filtered.rend(); ++it) {
    ...
    int availWidth = SCREEN_WIDTH - display->getStringWidth(timeBuf) - display->getStringWidth(chanType) - display->getStringWidth(" @...") - 10;
    if (availWidth < 0)
      availWidth = 0;
