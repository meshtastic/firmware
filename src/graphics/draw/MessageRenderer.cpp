  ...
  while (i < line.length()) {
    ...
    for (int e = 0; e < emoteCount; ++e) {
      ...
      if (pos != std::string::npos &&
          (nextEmotePos == std::string::npos || pos < nextEmotePos)) {
        nextEmotePos = pos;
    ...
    if (matchedEmote && i == nextEmotePos) {
      ...
      display->drawXbm(cursorX, iconY, matchedEmote->width,
                       matchedEmote->height, matchedEmote->bitmap);
...
void drawTextMessageFrame(OLEDDisplay *display, OLEDDisplayUiState *state,
  ...
  bool isInverted = (config.display.displaymode !=
  ...
  if (strlen(messageBuf) == 0) {
    ...
    int center_text =
        (SCREEN_WIDTH / 2) - (display->getStringWidth(messageString) / 2);
    display->drawString(center_text, getTextPositions(display)[2],
                        messageString);
    return;
  }
  ...
  uint32_t seconds = sinceReceived(&mp), minutes = seconds / 60,
           hours = minutes / 60, days = hours / 24;
  uint8_t timestampHours, timestampMinutes;
  int32_t daysAgo;
  bool useTimestamp =
      deltaToTimestamp(seconds, &timestampHours, &timestampMinutes, &daysAgo);

  if (useTimestamp && minutes >= 15 && daysAgo == 0) {
    std::string prefix =
        (daysAgo == 1 && SCREEN_WIDTH >= 200) ? "Yesterday" : "At";
    if (config.display.use_12h_clock) {
      bool isPM = timestampHours >= 12;
      timestampHours = timestampHours % 12;
      if (timestampHours == 0)
        timestampHours = 12;
      snprintf(headerStr, sizeof(headerStr), "%s %d:%02d%s from %s",
               prefix.c_str(), timestampHours, timestampMinutes,
               isPM ? "p" : "a", sender);
    } else {
      snprintf(headerStr, sizeof(headerStr), "%s %d:%02d from %s",
               prefix.c_str(), timestampHours, timestampMinutes, sender);
    }
  } else {
  ...
  for (int i = 0; i < numEmotes; ++i) {
    ...
    if (strcmp(msg, e.label) == 0) {
      int headerY =
          getTextPositions(display)[1]; // same as scrolling header line
      ...
      for (int separatorX = 0;
           ...
           separatorX += 2) {
        display->setPixel(separatorX,
                          headerY + ((SCREEN_WIDTH > 128) ? 19 : 13));
      }

      // Center the emote below the header line + separator + nav
      int remainingHeight =
          SCREEN_HEIGHT - (headerY + FONT_HEIGHT_SMALL) - navHeight;
      int emoteY = headerY + FONT_HEIGHT_SMALL +
                   (remainingHeight - e.height) / 2 + bounceY - bounceRange;
      display->drawXbm((SCREEN_WIDTH - e.width) / 2, emoteY, e.width, e.height,
                       e.bitmap);
      return;
    }
  }
