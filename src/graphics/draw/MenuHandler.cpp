...
BannerOverlayOptions createStaticBannerOptions(const char *message, const MenuOption<T> (&options)[N], std::array<const char *, N> &labels,
...
void menuHandler::LoraRegionPicker(uint32_t duration) {
  ...
  auto bannerOptions = createStaticBannerOptions(bannerMessage, regionOptions, regionLabels, [](const LoraRegionOption &option, int) -> void {
    if (!option.hasValue) {
      return;
    }
...
void menuHandler::DeviceRolePicker() {
  ...
  enum optionsNumbers { Back = 0, devicerole_client = 1, devicerole_clientmute = 2, devicerole_lostandfound = 3, devicerole_tracker = 4 };
  BannerOverlayOptions bannerOptions;
...
void menuHandler::RadioPresetPicker() {
  static const RadioPresetOption presetOptions[] = {{"Back", OptionsAction::Back},
  ...
  auto bannerOptions = createStaticBannerOptions("Radio Preset", presetOptions, presetLabels, [](const RadioPresetOption &option, int) -> void {
...
void menuHandler::ClockFacePicker() {
  ...
  auto bannerOptions = createStaticBannerOptions("Which Face?", clockFaceOptions, clockFaceLabels, [](const ClockFaceOption &option, int) -> void {
    if (option.action == OptionsAction::Back) {
      menuHandler::menuQueue = menuHandler::clock_menu;
      screen->runNow();
      return;
...
void menuHandler::TZPicker() {
  ...
  auto bannerOptions = createStaticBannerOptions("Pick Timezone", timezoneOptions, timezoneLabels, [](const TimezoneOption &option, int) -> void {
    if (option.action == OptionsAction::Back) {
      menuHandler::menuQueue = menuHandler::clock_menu;
      screen->runNow();
      return;
  ...
  for (size_t i = 0; i < timezoneCount; ++i) {
    if (timezoneOptions[i].hasValue && strncmp(config.device.tzdef, timezoneOptions[i].value, sizeof(config.device.tzdef)) == 0) {
      initialSelection = static_cast<int>(i);
      break;
    }
  }
...
void menuHandler::textMessageMenu() { cannedMessageModule->LaunchWithDestination(NODENUM_BROADCAST); }
...
void menuHandler::favoriteBaseMenu() {
  ...
  bannerOptions.bannerCallback = [](int selected) -> void {
    ...
    // Handle new Go To Thread action
      ...
      graphics::MessageRenderer::setThreadMode(graphics::MessageRenderer::ThreadMode::DIRECT, -1, graphics::UIRenderer::currentFavoriteNodeNum);
...
void menuHandler::positionBaseMenu() {
  enum class PositionAction { GpsToggle, GpsFormat, CompassMenu, CompassCalibrate, GPSSmartPosition, GPSUpdateInterval, GPSPositionBroadcast };
...
void menuHandler::nodeNameLengthMenu() {
  ...
  auto bannerOptions = createStaticBannerOptions("Node Name Length", nodeNameOptions, nodeNameLabels, [](const NodeNameOption &option, int) -> void {
    if (option.action == OptionsAction::Back) {
...
void menuHandler::compassNorthMenu() {
  ...
  auto bannerOptions = createStaticBannerOptions("North Directions?", compassOptions, compassLabels, [](const CompassOption &option, int) -> void {
    if (option.action == OptionsAction::Back) {
...
void menuHandler::GPSToggleMenu() {
  ...
  auto bannerOptions = createStaticBannerOptions("Toggle GPS", gpsToggleOptions, toggleLabels, [](const GPSToggleOption &option, int) -> void {
    if (option.action == OptionsAction::Back) {
      menuQueue = position_base_menu;
      screen->runNow();
      return;
...
void menuHandler::GPSUpdateIntervalMenu() {
  static const char *optionsArray[] = {"Back",       "8 seconds",  "20 seconds", "40 seconds", "1 minute", "80 seconds", "2 minutes", "5 minutes",
                                       "10 minutes", "15 minutes", "30 minutes", "1 hour",     "6 hours",  "12 hours",   "24 hours",  "At Boot Only"};
  BannerOverlayOptions bannerOptions;
...
void menuHandler::GPSPositionBroadcastMenu() {
  static const char *optionsArray[] = {"Back",    "1 minute", "90 seconds", "5 minutes", "15 minutes", "1 hour",   "2 hours",  "3 hours", "4 hours",
                                       "5 hours", "6 hours",  "12 hours",   "18 hours",  "24 hours",   "36 hours", "48 hours", "72 hours"};
  BannerOverlayOptions bannerOptions;
...
void menuHandler::TFTColorPickerMenu(OLEDDisplay *display) {
  ...
  auto bannerOptions =
      createStaticBannerOptions("Select Screen Color", colorOptions, colorLabels, [display](const ScreenColorOption &option, int) -> void {
        if (option.action == OptionsAction::Back) {
  ...
  bannerOptions.message = "Toggle Bluetooth";
        ...
        if (color.useVariant || (r == 0 && g == 0 && b == 0)) {
          ...
          uiconfig.screen_rgb_color = (static_cast<uint32_t>(r) << 16) | (static_cast<uint32_t>(g) << 8) | static_cast<uint32_t>(b);
        }
  ...
  if (uiconfig.screen_rgb_color == 0) {
    ...
    for (size_t i = 0; i < colorCount; ++i) {
      ...
      uint32_t encoded = (static_cast<uint32_t>(color.r) << 16) | (static_cast<uint32_t>(color.g) << 8) | static_cast<uint32_t>(color.b);
...
void menuHandler::numberTest() {
  screen->showNumberPicker("Pick a number\n ", 30000, 4, [](int number_picked) -> void { LOG_WARN("Nodenum: %u", number_picked); });
}
...
void menuHandler::keyVerificationInitMenu() {
  screen->showNodePicker("Node to Verify", 30000, [](uint32_t selected) -> void { keyVerificationModule->sendInitialRequest(selected); });
}
