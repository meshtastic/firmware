...
void drawDigitalClockFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) {
  ...
  if (!scaleInitialized) {
    ...
    float target_height = display->getHeight() -
                          (isHighResolution ? 46 : 33); // Be careful adjusting this number, we have to account for header and the text under the time
