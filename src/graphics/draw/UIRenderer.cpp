...
void UIRenderer::drawNavigationBar(OLEDDisplay *display, OLEDDisplayUiState *state)
{
    ...
    int usableWidth = display->getWidth() - (navPadding * 2);
    ...
    const int xStart = (display->getWidth() - totalWidth) / 2;

    const bool navBarVisible = millis() - lastFrameChangeTime <= ICON_DISPLAY_DURATION_MS;
    const int y = navBarVisible ? (display->getHeight() - iconSize - 1) : display->getHeight();
