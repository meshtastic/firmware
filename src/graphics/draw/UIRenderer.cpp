...
void UIRenderer::drawGpsPowerStatus(OLEDDisplay *display, int16_t x, int16_t y, const meshtastic::GPSStatus *gps) {
  ...
  if (y < FONT_HEIGHT_SMALL) { // Line 1: use short string
    ...
    displayLine = config.position.gps_mode == meshtastic_Config_PositionConfig_GpsMode_NOT_PRESENT ? "GPS not present" : "GPS is disabled";
...
void UIRenderer::drawGpsCoordinates(OLEDDisplay *display, int16_t x, int16_t y, const meshtastic::GPSStatus *gps, const char *mode) {
  ...
  if (!gps->getIsConnected() && !config.position.fixed_position) {
    ...
    if (gpsFormat != meshtastic_DeviceUIConfig_GpsCoordinateFormat_DMS) {
      ...
      if (gpsFormat == meshtastic_DeviceUIConfig_GpsCoordinateFormat_DEC) { // Decimal Degrees
        ...
        snprintf(coordinateLine_1, sizeof(coordinateLine_1), "%2i%1c %06u E", geoCoord.getUTMZone(), geoCoord.getUTMBand(), geoCoord.getUTMEasting());
        ...
        snprintf(coordinateLine_1, sizeof(coordinateLine_1), "%2i%1c %1c%1c", geoCoord.getMGRSZone(), geoCoord.getMGRSBand(),
                 geoCoord.getMGRSEast100k(), geoCoord.getMGRSNorth100k());
        snprintf(coordinateLine_2, sizeof(coordinateLine_2), "%05u E %05u N", geoCoord.getMGRSEasting(), geoCoord.getMGRSNorthing());
        ...
        if (geoCoord.getOSGRE100k() == 'I' || geoCoord.getOSGRN100k() == 'I') {     // OSGR is only valid around the UK region
          ...
          snprintf(coordinateLine_1, sizeof(coordinateLine_1), "%1c%1c", geoCoord.getOSGRE100k(), geoCoord.getOSGRN100k());
          snprintf(coordinateLine_2, sizeof(coordinateLine_2), "%05u E %05u N", geoCoord.getOSGREasting(), geoCoord.getOSGRNorthing());
        }
        ...
        snprintf(maiden, sizeof(maiden), "%c%c%c%c%c%c", 'A' + lonField, 'A' + latField, '0' + lonSquare, '0' + latSquare, 'A' + lonSub,
                 'A' + latSub);
      ...
      char coordinateLine_1[22];
      char coordinateLine_2[22];
      snprintf(coordinateLine_1, sizeof(coordinateLine_1), "Lat: %2i° %2i' %2u\" %1c", geoCoord.getDMSLatDeg(), geoCoord.getDMSLatMin(),
               geoCoord.getDMSLatSec(), geoCoord.getDMSLatCP());
      snprintf(coordinateLine_2, sizeof(coordinateLine_2), "Lon: %3i° %2i' %2u\" %1c", geoCoord.getDMSLonDeg(), geoCoord.getDMSLonMin(),
...
// Draw nodes status
void UIRenderer::drawNodes(OLEDDisplay *display, int16_t x, int16_t y, const meshtastic::NodeStatus *nodeStatus, int node_offset, bool show_total,
                           String additional_words) {
  char usersString[20];
  ...
  if (show_total) {
    ...
    snprintf(usersString, sizeof(usersString), "%d/%d %s", nodes_online, nodes_total, additional_words.c_str());
     ...
     defined(USE_ST7789) || defined(ILI9488_CS) || defined(HX8357_CS) || defined(ST7796_CS) || defined(HACKADAY_COMMUNICATOR) ||                     \
     defined(USE_ST7796)) &&                                                                                                                         \
...
void UIRenderer::drawNodeInfo(OLEDDisplay *display, const OLEDDisplayUiState *state, int16_t x, int16_t y) {
  ...
  if (SCREEN_WIDTH > SCREEN_HEIGHT) {
    ...
    if (showCompass) {
      int yBelowContent = (line > 0 && line <= 5) ? (getTextPositions(display)[line - 1] + FONT_HEIGHT_SMALL + 2) : getTextPositions(display)[1];
      const int margin = 4;
// --------- PATCH FOR EINK NAV BAR (ONLY CHANGE BELOW) -----------
      ...
      float myHeading = 0;
      if (uiconfig.compass_mode != meshtastic_CompassMode_FREEZE_HEADING) {
        myHeading = screen->hasHeading() ? screen->getHeading() * PI / 180 : screen->estimatedHeading(DegD(op.latitude_i), DegD(op.longitude_i));
      }
...
void UIRenderer::drawDeviceFocused(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) {
  ...
  if (config.position.gps_mode != meshtastic_Config_PositionConfig_GpsMode_ENABLED) {
    ...
    if (isHighResolution) {
      NodeListRenderer::drawScaledXBitmap16x16(x, getTextPositions(display)[line] + yOffset - 5, imgSatellite_width, imgSatellite_height,
                                               imgSatellite, display);
    } else {
  ...
  snprintf(shortnameble, sizeof(shortnameble), "%s", graphics::UIRenderer::haveGlyphs(owner.short_name) ? owner.short_name : "");
  ...
  display->drawString(starting_position + chUtil_x + chutil_bar_width + extraoffset, getTextPositions(display)[line], chUtilPercentage);
  ...
  if (SCREEN_WIDTH - (display->getStringWidth(combinedName)) > 10) {
    ...
    nameX = (SCREEN_WIDTH - textWidth) / 2;
    display->drawString(nameX, ((rows == 4) ? getTextPositions(display)[line++] : getTextPositions(display)[line++]) + yOffset, combinedName);
  } else {
    // === LongName Centered ===
...
void UIRenderer::drawIconScreen(const char *upperMsg, OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) {
  ...
  char buf[25];
  snprintf(buf, sizeof(buf), "%s   %s", xstr(APP_VERSION_SHORT), graphics::UIRenderer::haveGlyphs(owner.short_name) ? owner.short_name : "");
...
#else
  display->drawXbm(x + (SCREEN_WIDTH - icon_width) / 2, y + (SCREEN_HEIGHT - FONT_HEIGHT_MEDIUM - icon_height) / 2 + 2, icon_width, icon_height,
                   icon_bits);
  ...
  char buf[25];
  snprintf(buf, sizeof(buf), "%s\n%s", xstr(APP_VERSION_SHORT), graphics::UIRenderer::haveGlyphs(owner.short_name) ? owner.short_name : "");
...
void UIRenderer::drawCompassAndLocationScreen(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) {
  ...
  // === Update GeoCoord ===
  geoCoord.updateCoords(int32_t(gpsStatus->getLatitude()), int32_t(gpsStatus->getLongitude()), int32_t(gpsStatus->getAltitude()));
...
void UIRenderer::drawOEMIconScreen(const char *upperMsg, OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y) {
  ...
  if (isHighResolution) {
    ...
    display->drawXbm(x + (SCREEN_WIDTH - USERPREFS_OEM_IMAGE_WIDTH) / 2, y + (SCREEN_HEIGHT - USERPREFS_OEM_IMAGE_HEIGHT) / 2 + 2,
