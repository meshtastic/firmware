...
// key assignment when not in a free text entering state
static unsigned char TCA8418NavMapLong[_TCA8418_NUM_KEYS] = {
    Key::NONE,        // 1
    Key::NONE,        // 2
    Key::NONE,        // 3
    Key::NONE,        // 4
...
static Key getLongPressKey(uint8_t key_index, bool is_char_input_allowed)
{
    if (key_index >= _TCA8418_NUM_KEYS) {
        return Key::NONE;
    }

    return static_cast<Key>(is_char_input_allowed ? TCA8418LongPressMap[key_index] : TCA8418NavMapLong[key_index]);
}
...
void TCA8418Keyboard::held()
{
    ...
    bool is_char_input_allowed = cannedMessageModule && cannedMessageModule->isFreeTextActive();
    Key long_press_key = getLongPressKey(last_key, is_char_input_allowed);
    if (long_press_key == Key::NONE) {
        return;
    }
...
void TCA8418Keyboard::released()
{
    ...
    if (is_char_input_allowed) {
        if (tap_interval < _TCA8418_MULTI_TAP_THRESHOLD && should_backspace &&
            TCA8418TapMap[last_key][(char_idx % TCA8418TapMod[last_key])] != Key::BSP) {
            queueEvent(BSP);
        }
        if (held_interval > _TCA8418_LONG_PRESS_THRESHOLD) {
            if (!is_repeating_long_press && long_press_key != Key::NONE) {
                queueEvent(long_press_key);
    ...
    } else if (held_interval > _TCA8418_LONG_PRESS_THRESHOLD) {
        if (!is_repeating_long_press && long_press_key != Key::NONE) {
            queueEvent(long_press_key);
        }
