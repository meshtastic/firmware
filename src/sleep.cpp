...
void initDeepSleep() {
  ...
  LOG_INFO("Booted, wake cause %d (boot count %d), reset_reason=%s", wakeCause,
           bootCount, reason);
#endif
...
static void waitEnterSleep(bool skipPreflight = false) {
  if (!skipPreflight) {
    ...
    while (!doPreflightSleep()) {
      ...
      if (!Throttle::isWithinTimespanMs(
              now,
...
esp_sleep_wakeup_cause_t doLightSleep(uint64_t sleepMsec) {
...
#ifdef BUTTON_PIN
  gpio_num_t pin =
      (gpio_num_t)(config.device.button_gpio ? config.device.button_gpio
                                             : BUTTON_PIN);
...
#endif
#if !defined(SOC_PM_SUPPORT_EXT_WAKEUP) && defined(LORA_DIO1) &&               \
  ...
  notifyLightSleepEnd.notifyObservers(
      cause); // Button interrupts are reattached here

#ifdef BUTTON_PIN
  if (cause == ESP_SLEEP_WAKEUP_GPIO) {
    LOG_INFO("Exit light sleep gpio: btn=%d",
             !digitalRead(config.device.button_gpio ? config.device.button_gpio
                                                    : BUTTON_PIN));
  } else
#endif
  {
...
void enableLoraInterrupt() {
  esp_err_t res;
#if SOC_PM_SUPPORT_EXT_WAKEUP && defined(LORA_DIO1) &&                         \
    (LORA_DIO1 != RADIOLIB_NC)
