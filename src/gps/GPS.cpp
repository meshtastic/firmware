...
const uint8_t _message_PMREQ[] PROGMEM = {
    ...
    0x4D, 0x3B  // Placeholder for checksum, will be calculated next
};
...
uint8_t makeUBXCFG(uint8_t class_id, uint8_t msg_id, uint8_t msglen, const uint8_t *msg)
{
    if (msglen > (sizeof(UBXscratch) - 8)) {
        msglen = sizeof(UBXscratch) - 8;
    }
    ...
    UBXscratch[4] = msglen;   // length
    UBXscratch[5] = 0x00;

    UBXscratch[6 + msglen] = 0x00; // CK_A
    UBXscratch[7 + msglen] = 0x00; // CK_B

    for (int i = 2; i < 6; i++) {
        UBXscratch[6 + msglen] += UBXscratch[i];
        UBXscratch[7 + msglen] += UBXscratch[6 + msglen];
    }

    for (int i = 0; i < msglen; i++) {
        UBXscratch[6 + i] = pgm_read_byte(&msg[i]);
        UBXscratch[6 + msglen] += UBXscratch[6 + i];
        UBXscratch[7 + msglen] += UBXscratch[6 + msglen];
    }
    return (msglen + 8);
}
