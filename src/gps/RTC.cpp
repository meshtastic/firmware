...
RTCSetResult perhapsSetRTC(RTCQuality q, const struct timeval *tv, bool forceUpdate)
{
    ...
    if (tv->tv_sec < BUILD_EPOCH) {
        ...
        if (Throttle::isWithinTimespanMs(lastTimeValidationWarning, TIME_VALIDATION_WARNING_INTERVAL_MS) == false) {
            ...
            uint64_t maxAllowedTime64 = (uint64_t)BUILD_EPOCH + FORTY_YEARS;
            LOG_DEBUG("GPS time validation: BUILD_EPOCH=%lu, maxAllowedTime64=%lu, UINT32_MAX=%lu", (unsigned long)BUILD_EPOCH,
                      (unsigned long)maxAllowedTime64, (unsigned long)UINT32_MAX);
