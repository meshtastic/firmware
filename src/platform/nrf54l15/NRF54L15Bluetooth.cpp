...
static const struct bt_uuid_128 mesh_svc_uuid =
    BT_UUID_INIT_128(MESH_SVC_UUID_VAL);
static const struct bt_uuid_128 fromnum_uuid =
    BT_UUID_INIT_128(FROMNUM_UUID_VAL);
static const struct bt_uuid_128 fromradio_uuid =
    BT_UUID_INIT_128(FROMRADIO_UUID_VAL);
static const struct bt_uuid_128 toradio_uuid =
    BT_UUID_INIT_128(TORADIO_UUID_VAL);
static const struct bt_uuid_128 logradio_uuid =
    BT_UUID_INIT_128(LOGRADIO_UUID_VAL);
...
static void fromnum_ccc_changed(const struct bt_gatt_attr *attr,
                                uint16_t value) {
  fromnum_ccc_val = value;
  LOG_INFO("BLE fromNum CCC: %u", value);
}

static void logradio_ccc_changed(const struct bt_gatt_attr *attr,
                                 uint16_t value) {
...
static ssize_t read_fromnum(struct bt_conn *conn,
                            const struct bt_gatt_attr *attr, void *buf,
                            uint16_t len, uint16_t offset) {
  LOG_INFO("GATT read_fromnum: fromNum=%u offset=%u", fromNumValue, offset);
  return bt_gatt_attr_read(conn, attr, buf, len, offset, &fromNumValue,
                           sizeof(fromNumValue));
}

static ssize_t read_fromradio(struct bt_conn *conn,
                              const struct bt_gatt_attr *attr, void *buf,
                              uint16_t len, uint16_t offset) {
  if (offset == 0) {
  ...
  return bt_gatt_attr_read(conn, attr, buf, len, offset, fromRadioBytes,
                           fromRadioLen);
}

static ssize_t read_logradio(struct bt_conn *conn,
                             const struct bt_gatt_attr *attr, void *buf,
                             uint16_t len, uint16_t offset) {
...
static ssize_t write_toradio(struct bt_conn *conn,
...
BT_GATT_SERVICE_DEFINE(
    ...
    BT_GATT_CHARACTERISTIC(&fromnum_uuid.uuid,
                           ...
                           MESH_PERM_READ, read_fromnum, NULL, &fromNumValue),
    ...
    BT_GATT_CHARACTERISTIC(&fromradio_uuid.uuid, BT_GATT_CHRC_READ,
                           MESH_PERM_READ, read_fromradio, NULL, NULL),

    // toRadio: WRITE — phone sends protobuf packets to the device
    BT_GATT_CHARACTERISTIC(&toradio_uuid.uuid, BT_GATT_CHRC_WRITE,
                           MESH_PERM_WRITE, NULL, write_toradio, NULL),
...
static void stop_advertising() { bt_le_adv_stop(); }
...
static void security_changed_cb(struct bt_conn *conn, bt_security_t level,
  ...
  if (err == BT_SECURITY_ERR_PIN_OR_KEY_MISSING) {
    ...
    LOG_WARN("BLE security change failed: level=%d err=%d", (int)level,
             (int)err);
  } else {
...
void BluetoothPhoneAPI::onNowHasData(uint32_t fromRadioNum) {
  ...
  if (!conn)
    return;
  bt_gatt_notify(conn, &mesh_svc.attrs[FROMNUM_ATTR_IDX], &fromNumValue,
                 sizeof(fromNumValue));
  bt_conn_unref(conn);
}

bool BluetoothPhoneAPI::checkIsConnected() { return active_conn != nullptr; }
...
class BleDeferredThread : public concurrency::OSThread {
  static constexpr uint32_t IDLE_BEFORE_PROBE_MS =
      30000;                                          //  30 s: start probing
...
protected:
  int32_t runOnce() override {
    ...
    if (att_idle > IDLE_BEFORE_PROBE_MS &&
      ...
      last_probe_ms = now;
      int err = bt_gatt_notify(conn, &mesh_svc.attrs[FROMNUM_ATTR_IDX],
...
bool NRF54L15Bluetooth::isConnected() { return active_conn != nullptr; }
...
void NRF54L15Bluetooth::sendLog(const uint8_t *logMessage, size_t length) {
  ...
  bt_gatt_notify(active_conn, &mesh_svc.attrs[LOGRADIO_ATTR_IDX], logMessage,
                 (uint16_t)length);
}
