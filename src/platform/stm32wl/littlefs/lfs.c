...
static int lfs_cache_read(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache, lfs_block_t block, lfs_off_t off, void *buffer,
                          lfs_size_t size) {
  uint8_t *data = buffer;
...
static int lfs_cache_cmp(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache, lfs_block_t block, lfs_off_t off, const void *buffer,
                         lfs_size_t size) {
...
static int lfs_cache_crc(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache, lfs_block_t block, lfs_off_t off, lfs_size_t size,
                         uint32_t *crc) {
  for (lfs_off_t i = 0; i < size; i++) {
    uint8_t c;
    int err = lfs_cache_read(lfs, rcache, pcache, block, off + i, &c, 1);
...
static int lfs_cache_prog(lfs_t *lfs, lfs_cache_t *pcache, lfs_cache_t *rcache, lfs_block_t block, lfs_off_t off, const void *buffer,
                          lfs_size_t size) {
  const uint8_t *data = buffer;
  LFS_ASSERT(block < lfs->cfg->block_count);
...
static int lfs_bd_erase(lfs_t *lfs, lfs_block_t block) { return lfs->cfg->erase(lfs->cfg, block); }
...
static void lfs_alloc_ack(lfs_t *lfs) { lfs->free.ack = lfs->cfg->block_count; }
...
static inline bool lfs_pairisnull(const lfs_block_t pair[2]) { return pair[0] == 0xffffffff || pair[1] == 0xffffffff; }
...
static inline lfs_size_t lfs_entry_size(const lfs_entry_t *entry) { return 4 + entry->d.elen + entry->d.alen + entry->d.nlen; }
...
static int lfs_dir_commit(lfs_t *lfs, lfs_dir_t *dir, const struct lfs_region *regions, int count) {
  ...
  if (relocated) {
    ...
    LFS_DEBUG("Relocating %" PRIu32 " %" PRIu32 " to %" PRIu32 " %" PRIu32, oldpair[0], oldpair[1], dir->pair[0], dir->pair[1]);
...
static int lfs_dir_append(lfs_t *lfs, lfs_dir_t *dir, lfs_entry_t *entry, const void *data) {
  ...
  while (true) {
    if (dir->d.size + lfs_entry_size(entry) <= lfs->cfg->block_size) {
      ...
      lfs_entry_tole32(&entry->d);
      int err =
          lfs_dir_commit(lfs, dir, (struct lfs_region[]){{entry->off, 0, &entry->d, sizeof(entry->d)}, {entry->off, 0, data, entry->d.nlen}}, 2);
      lfs_entry_fromle32(&entry->d);
      return err;
    }
    ...
    if (!(0x80000000 & dir->d.size)) {
      ...
      entry->off = dir->d.size - 4;
      lfs_entry_tole32(&entry->d);
      err = lfs_dir_commit(lfs, dir, (struct lfs_region[]){{entry->off, 0, &entry->d, sizeof(entry->d)}, {entry->off, 0, data, entry->d.nlen}}, 2);
      lfs_entry_fromle32(&entry->d);
      if (err) {
        return err;
      }
...
static int lfs_ctz_find(lfs_t *lfs, lfs_cache_t *rcache, const lfs_cache_t *pcache, lfs_block_t head, lfs_size_t size, lfs_size_t pos,
...
static int lfs_ctz_extend(lfs_t *lfs, lfs_cache_t *rcache, lfs_cache_t *pcache, lfs_block_t head, lfs_size_t size, lfs_block_t *block,
                          lfs_off_t *off) {
  while (true) {
...
int lfs_file_open(lfs_t *lfs, lfs_file_t *file, const char *path, int flags) { return lfs_file_opencfg(lfs, file, path, flags, NULL); }
...
int lfs_format(lfs_t *lfs, const struct lfs_config *cfg) {
  ...
  if (true) {
    ...
    bool valid = false;
    for (int i = 0; i < 2; i++) {
      err = lfs_dir_commit(lfs, &superdir, (struct lfs_region[]){{sizeof(superdir.d), sizeof(superblock.d), &superblock.d, sizeof(superblock.d)}}, 1);
      if (err && err != LFS_ERR_CORRUPT) {
        goto cleanup;
      }
