...
void cpuDeepSleep(uint32_t msecs) { notImplemented("cpuDeepSleep"); }
...
void portduinoSetup() {
  ...
  if (portduino_config.config_directory != "") {
    ...
    for (const std::filesystem::directory_entry &entry : std::filesystem::directory_iterator{portduino_config.config_directory}) {
  ...
  if (portduino_config.lora_module == use_autoconf) {
    ...
    try {
      ...
      ch341Hal = new Ch341Hal(0, portduino_config.lora_usb_serial_num, portduino_config.lora_usb_vid, portduino_config.lora_usb_pid);
    ...
    if (strlen(autoconf_product) < 6) {
      ...
      if (access("/proc/device-tree/hat/product", R_OK) == 0) {
        ...
        std::cout << "autoconf: Found Pi HAT+ " << hat_vendor << " " << autoconf_product << " at /proc/device-tree/hat" << std::endl;
        found_hat = true;
      } else {
    ...
    if (strlen(autoconf_product) > 0) {
      ...
      if (configProducts.find(autoconf_product) != configProducts.end()) {
        ...
        if (found_hat) {
          product_config = cleanupNameForAutoconf("lora-hat-" + std::string(hat_vendor) + "-" + autoconf_product + ".yaml");
        } else if (found_ch341) {
        ...
        if (access((portduino_config.available_directory + product_config).c_str(), R_OK) != 0) {
          std::cerr << "autoconf: Unable to find config for " << autoconf_product << "(tried " << product_config << ")" << std::endl;
          exit(EXIT_FAILURE);
        }
      }
      ...
      if (loadConfig((portduino_config.available_directory + product_config).c_str())) {
        ...
        std::cerr << "autoconf: Unable to use " << product_config << " as config file for " << autoconf_product << std::endl;
        exit(EXIT_FAILURE);
      }
    } else {
...
bool loadConfig(const char *configPath) {
  ...
  try {
    ...
    if (yamlConfig["Lora"]) {
      ...
      if (portduino_config.lora_module != use_autoconf && portduino_config.lora_module != use_simradio && !portduino_config.force_simradio) {
    ...
    if (yamlConfig["Webserver"]) {
      ...
      portduino_config.webserver_root_path = (yamlConfig["Webserver"]["RootPath"]).as<std::string>("/usr/share/meshtasticd/web");
      portduino_config.webserver_ssl_key_path = (yamlConfig["Webserver"]["SSLKey"]).as<std::string>("/etc/meshtasticd/ssl/private_key.pem");
      portduino_config.webserver_ssl_cert_path = (yamlConfig["Webserver"]["SSLCert"]).as<std::string>("/etc/meshtasticd/ssl/certificate.pem");
    }
    ...
    if (yamlConfig["General"]) {
      ...
      portduino_config.available_directory = (yamlConfig["General"]["AvailableDirectory"]).as<std::string>("/etc/meshtasticd/available.d/");
      if ((yamlConfig["General"]["MACAddress"]).as<std::string>("") != "" && (yamlConfig["General"]["MACAddressSource"]).as<std::string>("") != "") {
      ...
      portduino_config.mac_address.erase(std::remove(portduino_config.mac_address.begin(), portduino_config.mac_address.end(), ':'),
...
std::string exec(const char *cmd) { // https://stackoverflow.com/a/478960
