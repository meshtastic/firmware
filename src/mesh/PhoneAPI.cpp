...
bool PhoneAPI::handleToRadioPacket(meshtastic_MeshPacket &p) {
  ...
  if (p.decoded.portnum == meshtastic_PortNum_TRACEROUTE_APP && lastPortNumToRadio[p.decoded.portnum] &&
    ...
    service->sendQueueStatusToPhone(qs, 0, p.id);
    return false;
  } else if (IS_ONE_OF(p.decoded.portnum, meshtastic_PortNum_POSITION_APP, meshtastic_PortNum_WAYPOINT_APP, meshtastic_PortNum_ALERT_APP,
                       meshtastic_PortNum_TELEMETRY_APP) &&
             lastPortNumToRadio[p.decoded.portnum] && Throttle::isWithinTimespanMs(lastPortNumToRadio[p.decoded.portnum], TEN_SECONDS_MS)) {
