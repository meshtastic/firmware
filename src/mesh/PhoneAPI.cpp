...
size_t PhoneAPI::getFromRadio(uint8_t *buf)
{
    ...
    case STATE_SEND_PACKETS:
        ...
        if (queueStatusPacketForPhone) {
            ...
            // Always deliver clientNotification — required for LOCKDOWN_UNLOCKED/LOCKDOWN_LOCKED to reach client
...
void PhoneAPI::sendConfigComplete()
{
    ...
    if (!isAdminAuthorized && !s_localAdminAuthorized) {
        char msg[48];
        if (!EncryptedStorage::isProvisioned()) {
            strncpy(msg, "LOCKDOWN_NEEDS_PROVISION", sizeof(msg));
        } else {
            ...
            if (!EncryptedStorage::isUnlocked()) {
                snprintf(msg, sizeof(msg), "LOCKDOWN_LOCKED:%s", EncryptedStorage::getLockReason());
            } else {
                strncpy(msg, "LOCKDOWN_LOCKED:needs_auth", sizeof(msg));
            }
        }
...
void PhoneAPI::authorizeLocalAdmin()
{
    ...
    LOG_INFO("Lockdown: Local admin authorized via PKC");
}
