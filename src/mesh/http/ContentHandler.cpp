...
void handleFormUpload(HTTPRequest *req, HTTPResponse *res) {
  ...
  while (parser->nextField()) {
    ...
    LOG_DEBUG("handleFormUpload: field name='%s', filename='%s', mimetype='%s'", name.c_str(), filename.c_str(), mimeType.c_str());
...
void handleAdminSettingsApply(HTTPRequest *req, HTTPResponse *res) {
  ...
  res->println("<h1>Meshtastic</h1>");
  res->println("<html><head><meta http-equiv=\"refresh\" content=\"1;url=/admin/settings\" /><title>Settings Applied. </title>");
