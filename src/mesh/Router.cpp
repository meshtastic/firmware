...
meshtastic_Routing_Error perhapsEncode(meshtastic_MeshPacket *p) {
  ...
  if (p->which_payload_variant == meshtastic_MeshPacket_decoded_tag) {
    ...
    if (isFromUs(p) &&
      ...
      crypto->encryptCurve25519(p->to, getFrom(p), node->user.public_key, p->id,
                                numbytes, bytes, p->encrypted.bytes);
...
void Router::handleReceived(meshtastic_MeshPacket *p, RxSource src) {
  ...
  if (!skipHandle) {
    ...
    if (p_encrypted == nullptr) {
      ...
      // After potentially altering it, publish received message to MQTT if
      // we're not the original transmitter of the packet
