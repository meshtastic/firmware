...
static NodeInfoBootstrapResult verifyFirstContactNodeInfo(meshtastic_MeshPacket *p)
{
    ...
    // Reject reserved senders before any NodeDB entry exists: isBroadcast() is true for
    // NODENUM_BROADCAST_NO_LORA, so a DM to such a contact would silently skip PKI.
