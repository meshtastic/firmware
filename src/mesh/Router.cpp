...
DecodeState perhapsDecode(meshtastic_MeshPacket *p)
{
    ...
    if (p->channel == 0 && isToUs(p) && p->to > 0 && !isBroadcast(p->to) && nodeDB->getMeshNode(p->from) != nullptr &&
        ...
        pki_decrypted = crypto->decryptCurve25519(p->from, nodeDB->getMeshNode(p->from)->user.public_key, p->id, rawSize,
        ...
        if (pki_decrypted) {
            ...
            if (ephemeralKeyMgr && ephemeralKeyMgr->isInitialized()) {
                ...
                if (senderEphKey && ephemeralKeyMgr->nodeSuportsPFS(p->from)) {
                    ...
                    pki_decrypted = crypto->decryptWithPFS(p->from, nodeDB->getMeshNode(p->from)->user.public_key,
        ...
        if (isFromUs(p) &&
            ...
            if (ephemeralKeyMgr && ephemeralKeyMgr->isInitialized()) {
                ...
                if (remoteEphKey && ephemeralKeyMgr->nodeSuportsPFS(p->to)) {
                    ...
                    encrypted = crypto->encryptWithPFS(p->to, getFrom(p), node->user.public_key, remoteEphKey->pubkey, p->id,
