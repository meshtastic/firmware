...
NodeDB::NodeDB() {
  ...
  if (channels.isDefaultChannel(channels.getPrimaryIndex())) {
    ...
    moduleConfig.telemetry.device_update_interval =
        Default::getConfiguredOrMinimumValue(moduleConfig.telemetry.device_update_interval, min_default_telemetry_interval_secs);
    moduleConfig.telemetry.environment_update_interval =
        Default::getConfiguredOrMinimumValue(moduleConfig.telemetry.environment_update_interval, min_default_telemetry_interval_secs);
    moduleConfig.telemetry.air_quality_interval =
        Default::getConfiguredOrMinimumValue(moduleConfig.telemetry.air_quality_interval, min_default_telemetry_interval_secs);
    moduleConfig.telemetry.power_update_interval =
        Default::getConfiguredOrMinimumValue(moduleConfig.telemetry.power_update_interval, min_default_telemetry_interval_secs);
    moduleConfig.telemetry.health_update_interval =
        Default::getConfiguredOrMinimumValue(moduleConfig.telemetry.health_update_interval, min_default_telemetry_interval_secs);
  }
...
bool isFromUs(const meshtastic_MeshPacket *p) { return p->from == 0 || p->from == nodeDB->getNodeNum(); }

// Returns true if the packet is destined to us
bool isToUs(const meshtastic_MeshPacket *p) { return p->to == nodeDB->getNodeNum(); }

bool isBroadcast(uint32_t dest) { return dest == NODENUM_BROADCAST || dest == NODENUM_BROADCAST_NO_LORA; }
...
void NodeDB::installDefaultConfig(bool preserveKey = false) {
  ...
  if (IS_ONE_OF(USERPREFS_CONFIG_DEVICE_ROLE, meshtastic_Config_DeviceConfig_Role_ROUTER, meshtastic_Config_DeviceConfig_Role_ROUTER_LATE,
...
#if defined(ST7735_CS) || defined(USE_EINK) || defined(ILI9341_DRIVER) || defined(ILI9342_DRIVER) || defined(ST7789_CS) || defined(HX8357_CS) ||     \
    defined(USE_ST7789) || defined(ILI9488_CS) || defined(ST7796_CS) || defined(USE_SPISSD1306) || defined(USE_ST7796) ||                            \
...
void NodeDB::installRoleDefaults(meshtastic_Config_DeviceConfig_Role role) {
  if (role == meshtastic_Config_DeviceConfig_Role_ROUTER) {
    ...
    config.position.position_flags = (meshtastic_Config_PositionConfig_PositionFlags_ALTITUDE | meshtastic_Config_PositionConfig_PositionFlags_SPEED |
...
LoadFileResult NodeDB::loadProto(const char *filename, size_t protoSize, size_t objSize, const pb_msgdesc_t *fields, void *dest_struct) {
...
void NodeDB::loadFromDisk() {
...
#endif
  auto state =
      loadProto(nodeDatabaseFileName, getMaxNodesAllocatedSize(), sizeof(meshtastic_NodeDatabase), &meshtastic_NodeDatabase_msg, &nodeDatabase);
  ...
  state = loadProto(configFileName, meshtastic_LocalConfig_size, sizeof(meshtastic_LocalConfig), &meshtastic_LocalConfig_msg, &config);
  ...
  state = loadProto(moduleConfigFileName, meshtastic_LocalModuleConfig_size, sizeof(meshtastic_LocalModuleConfig), &meshtastic_LocalModuleConfig_msg,
                    &moduleConfig);
  if (state != LoadFileResult::LOAD_SUCCESS) {
  ...
  state = loadProto(channelFileName, meshtastic_ChannelFile_size, sizeof(meshtastic_ChannelFile), &meshtastic_ChannelFile_msg, &channelFile);
  if (state != LoadFileResult::LOAD_SUCCESS) {
  ...
  state = loadProto(uiconfigFileName, meshtastic_DeviceUIConfig_size, sizeof(meshtastic_DeviceUIConfig), &meshtastic_DeviceUIConfig_msg, &uiconfig);
  if (state == LoadFileResult::LOAD_SUCCESS) {
...
bool NodeDB::saveProto(const char *filename, size_t protoSize, const pb_msgdesc_t *fields, const void *dest_struct, bool fullAtomic) {
  bool okay = false;
#ifdef FSCom
...
void NodeDB::updatePosition(uint32_t nodeId, const meshtastic_Position &p, RxSource src) {
  ...
  if (src == RX_SRC_LOCAL) {
    ...
    LOG_INFO("updatePosition LOCAL pos@%x time=%u lat=%d lon=%d alt=%d", p.timestamp, p.time, p.latitude_i, p.longitude_i, p.altitude);
...
bool NodeDB::updateUser(uint32_t nodeId, meshtastic_User &p, uint8_t channelIndex) {
  ...
  LOG_DEBUG("Update changed=%d user %s/%s, id=0x%08x, channel=%d", changed, info->user.long_name, info->user.short_name, nodeId, info->channel);
  info->has_user = true;
    ...
    while (changed) { // dumb reverse bubble sort, but probably not bad for what we're doing
      ...
      for (int i = numMeshNodes - 1; i > 0; i--) { // lowest case this should examine is i == 1
        if (meshNodes->at(i - 1).num == getNodeNum()) {
          // noop
        } else if (meshNodes->at(i).num == getNodeNum()) { // in the oddball case our own node num is not at location 0, put it there
...
bool NodeDB::isFull() { return (numMeshNodes >= MAX_NUM_NODES) || (memGet.getFreeHeap() < MINIMUM_SAFE_FREE_HEAP); }
    ...
    if (isFull()) {
      ...
      for (int i = 1; i < numMeshNodes; i++) {
        ...
        if (!meshNodes->at(i).is_favorite && !meshNodes->at(i).is_ignored &&
            !(meshNodes->at(i).bitfield & NODEINFO_BITFIELD_IS_KEY_MANUALLY_VERIFIED_MASK) && meshNodes->at(i).last_heard < oldest) {
...
bool NodeDB::restorePreferences(meshtastic_AdminMessage_BackupLocation location, int restoreWhat) {
  ...
  if (location == meshtastic_AdminMessage_BackupLocation_FLASH) {
    ...
    meshtastic_BackupPreferences backup = meshtastic_BackupPreferences_init_zero;
    success = loadProto(backupFileName, meshtastic_BackupPreferences_size, sizeof(meshtastic_BackupPreferences), &meshtastic_BackupPreferences_msg,
                        &backup);
    if (success) {
