...
void NodeDB::addFromContact(meshtastic_SharedContact contact)
{
    ...
    // CopyUserToNodeInfoLite assigns public_key unconditionally. A contact may supply or update a
    // full key, but must never erase one we already hold - clients send add_contact before every DM.
