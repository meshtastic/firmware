...
template <typename T>
ServerAPI<T>::ServerAPI(T &_client)
    : StreamAPI(&client), concurrency::OSThread("ServerAPI"), client(_client) {
...
template <typename T> bool ServerAPI<T>::canWriteFrame(size_t frameLen) {
  ...
  if (writable != -1 && writable <= 0) {
    canWrite = false;
    enabled = false;
    LOG_WARN("TCP client not writable before write (%d/%lu bytes), closing API "
             "service",
             writable, (unsigned long)frameLen);
    close();
    return false;
  }
...
template <typename T>
void ServerAPI<T>::onFrameWriteFailed(size_t frameLen, size_t writtenLen) {
  canWrite = false;
  enabled = false;
  LOG_WARN("TCP client write short (%lu/%lu bytes), closing API service",
           (unsigned long)writtenLen, (unsigned long)frameLen);
  close();
}

template <class T> int32_t ServerAPI<T>::runOnce() {
  if (client.connected()) {
    if (lastContactMsec > 0 &&
        !Throttle::isWithinTimespanMs(lastContactMsec, TCP_IDLE_TIMEOUT_MS)) {
      LOG_WARN("TCP connection timeout, no data for %lu ms",
               (unsigned long)(millis() - lastContactMsec));
      close();
      enabled = false;
      return 0;
    }
...
APIServerPort<T, U>::APIServerPort(int port)
    : U(port), concurrency::OSThread("ApiServer") {}
...
template <class T, class U> int32_t APIServerPort<T, U>::runOnce() {
  ...
  if (client) {
    ...
    if (openAPI) {
      ...
      if (waitTime < 400) {
        waitTime *= 2;
        LOG_INFO("Previous TCP connection still open, try again in %dms",
                 waitTime);
        return waitTime;
      }
#endif
