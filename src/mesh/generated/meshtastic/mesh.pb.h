...
typedef struct _meshtastic_IdAttestation {
 ...
 by recipients. */
    uint32_t subject;
    /* Only KNOWN_SINCE: the attester's belief about when it last observed
 ...
 overflow. L1 populates it from the attester's NodeDB last-heard record
 (a proxy for first-seen, which L2 identity will replace). A value of 0
 means "the attester has no clock"; recipients then fall back to the
 tenure floor (attestation_min_tenure_secs) only. */
    uint32_t known_since_secs;
    /* Only KNOWN_SINCE: the attester's own device uptime in seconds (how long
 this attester has been running, i.e. the maximum possible local tenure).
 Lets a recipient reject attestations from an attester younger than
 attestation_min_tenure_secs. 0 means "no clock". */
