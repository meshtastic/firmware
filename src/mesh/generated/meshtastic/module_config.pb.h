...
typedef struct _meshtastic_ModuleConfig_TrafficManagementConfig {
 ...
 in probation. */
    ...
    /* Relay pricing: when a gossiped NO_RELAY bit for `subject` arrives,
 honor it only if this device has itself seen `subject` over-relay the
 current window (its local relay budget for `subject` is exhausted or
 it is rate-limited here). 1 requires that local observation; 0
 accepts the bit on its own word. Non-zero is the shipped default:
 free-floating claims do not stop a relay. */
    uint32_t no_relay_requires_local_exhaustion;
    /* Relay pricing: maximum number of distinct senders one attester may
 mark NO_RELAY for per rate window. A claim for a (sender, attester)
 pair older than no_relay_ttl_secs does not count against the cap
 again. 0 disables the per-attester cap - not the shipped default
 (3). */
    uint32_t no_relay_max_subjects_per_window;
    /* Relay pricing: lifetime in seconds of one gossiped NO_RELAY claim
 before the attester may re-claim that sender. Keep below
 rate_limit_window_secs so the claim is window-scoped. 0 disables the
 re-claim TTL - not the shipped default (120). */
    uint32_t no_relay_ttl_secs;
    /* Greylist: minimum seconds this device must have actually observed
 the attester (first-seen tracking, independent of the attester's
 self-reported uptime) before a known-since promotion from it is
 accepted. The self-reported tenure floor
 (attestation_min_tenure_secs) still applies; this observed check is
 the binding one. 0 disables the observed check - not the shipped
 default (86400). */
    uint32_t attestation_min_observed_secs;
    /* Greylist: how many times per rate window this device may vouch for
 one subject, and how many times a receiver may accept one attester's
 vouches for the same subject per window. 0 disables the per-subject
 cap - not the shipped default (1). */
    uint32_t vouch_max_per_subject_per_window;
    /* Greylist: maximum number of distinct subjects one attester may vouch
 for per rate window, enforced on the emitting device and on the
 ...
 probation when vouch cadence is sparser than the lease. */
    ...
    /* Trust ladder: minimum seconds of local OBSERVED age an attester must
 have for its vouch to use the signed fast path - a verified signature
 replaces the self-reported tenure claim, and the vouch then grants an
 L2 (neighbor-attested) upgrade instead of the L1 (TOFU-signed) one.
 The same floor is the ladder's decay window: an L2 upgrade lapses back
 to L1 once the subject stops sending verified signatures for this
 long. 0 falls back to the attestation_min_observed_secs observed-tenure
 floor for both (matching old firmware configs) - not the shipped
 default. */
...
#define meshtastic_ModuleConfig_TrafficManagementConfig_size 141
