...
/**
 * Derive a session key using Triple Diffie-Hellman.
 *
 * This combines three DH operations:
 *   DH1 = DH(identity_local_priv, ephemeral_remote_pub)
 *   DH2 = DH(ephemeral_local_priv, identity_remote_pub)
 *   DH3 = DH(ephemeral_local_priv, ephemeral_remote_pub)
 *
 * The session key is derived by hashing: DH1 || DH2 || DH3
 */
bool CryptoEngine::deriveTripleDHSessionKey(
    const uint8_t *remoteIdentityKey, const uint8_t *remoteEphemeralKey,
    const uint8_t *localEphemeralPrivKey, uint8_t *sessionKeyOut) {
  uint8_t dh1[32], dh2[32], dh3[32];
  uint8_t combined[96]; // 3 * 32 bytes

  // DH1: local identity private key with remote ephemeral public key
  if (!Curve25519::eval(dh1, private_key, remoteEphemeralKey)) {
    LOG_WARN("DH1 failed: invalid remote ephemeral key");
    return false;
  }

  // DH2: local ephemeral private key with remote identity public key
  if (!Curve25519::eval(dh2, localEphemeralPrivKey, remoteIdentityKey)) {
    LOG_WARN("DH2 failed: invalid remote identity key");
    return false;
  }

  // DH3: local ephemeral private key with remote ephemeral public key
  if (!Curve25519::eval(dh3, localEphemeralPrivKey, remoteEphemeralKey)) {
    LOG_WARN("DH3 failed: ephemeral key exchange failed");
    return false;
  }

  // Combine all three DH outputs
  ...
  LOG_DEBUG("Triple-DH session key derived successfully");
  return true;
}
...
bool CryptoEngine::encryptWithPFS(
  ...
  // Get local ephemeral private key from EphemeralKeyManager
...
bool CryptoEngine::decryptWithPFS(
  ...
  if (!ephemeralKeyMgr || !ephemeralKeyMgr->isInitialized()) {
    ...
    return false; // Cannot decrypt - no fallback possible
  }
