...
struct __attribute__((packed)) OTAHeader {
  ...
  uint32_t
      firmwareSize; // Size of the firmware payload in bytes (little-endian)
...
static const uint32_t OTA_AUTH_COOLDOWN_MS =
    5000; // 5s cooldown after failed auth
...
static const uint8_t *const otaPSK =
    reinterpret_cast<const uint8_t *>(otaPSKString);
static const size_t otaPSKSize = sizeof(otaPSKString) - 1;
#else
// Default PSK (CHANGE THIS for production deployments)
static const uint8_t otaPSK[] = {
    0x6d, 0x65, 0x73, 0x68, 0x74, 0x61, 0x73, 0x74, 0x69, 0x63, 0x5f,
...
static void computeAuthHash(const uint8_t *nonce, size_t nonceLen,
...
static bool authenticateClient(EthernetClient &client) {
  ...
  if (lastAuthFailure != 0 &&
      (millis() - lastAuthFailure) < OTA_AUTH_COOLDOWN_MS) {
...
static void handleOTAClient(EthernetClient &client) {
  LOG_INFO("ETH OTA: Client connected from %u.%u.%u.%u", client.remoteIP()[0],
  ...
  LOG_INFO("ETH OTA: Firmware size=%u, CRC32=0x%08X", hdr.firmwareSize,
           hdr.crc32);
  ...
  while (remaining > 0) {
    ...
    if (avail <= 0) {
      if (millis() - lastActivity > OTA_TIMEOUT_MS) {
        LOG_WARN("ETH OTA: Timeout during transfer (%u/%u bytes)",
                 totalReceived, hdr.firmwareSize);
    ...
    if (written != got) {
      LOG_ERROR("ETH OTA: Write failed (wrote %u of %u), error=%u", written,
                got, Update.getError());
    ...
    if (totalReceived % (hdr.firmwareSize / 10 + 1) < got) {
      LOG_INFO("ETH OTA: %u%% (%u/%u bytes)",
               (uint32_t)(100ULL * totalReceived / hdr.firmwareSize),
               totalReceived, hdr.firmwareSize);
    }
  }
  ...
  if (computedCRC != hdr.crc32) {
    LOG_ERROR("ETH OTA: CRC mismatch (expected=0x%08X, computed=0x%08X)",
              hdr.crc32, computedCRC);
  ...
  LOG_INFO("ETH OTA: Update staged successfully (%u bytes). Rebooting...",
           hdr.firmwareSize);
  client.write(OTA_OK);
  client.flush();
  delay(500);
