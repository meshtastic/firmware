...
int MeshService::handleFromRadio(const meshtastic_MeshPacket *mp)
{
    ...
    if (mp->decoded.portnum == meshtastic_PortNum_TELEMETRY_APP && mp->decoded.request_id > 0) {
        LOG_DEBUG(
            "Received telemetry response. Don't send our NodeInfo because this potentially a Repeater which will ignore our "
