...
int MeshService::handleFromRadio(const meshtastic_MeshPacket *mp)
{
    ...
    if (mp->which_payload_variant == meshtastic_MeshPacket_decoded_tag &&
        ...
        // 1. Determine safe length (Protobuf payloads aren't always null-terminated)
        size_t incomingLen = mp->decoded.payload.size;
        
        // 2. Prevent infinite loops (only echo if it doesn't start with "echo: ")
        // We use memcmp because incomingText might not be null-terminated yet
        if (incomingLen > 0 && (incomingLen < 6 || memcmp(mp->decoded.payload.bytes, "echo: ", 6) != 0)) {
            
            char echoBuf[meshtastic_Constants_DATA_PAYLOAD_LEN]; 
            int written = snprintf(echoBuf, sizeof(echoBuf), "echo: %.*s", (int)incomingLen, (const char*)mp->decoded.payload.bytes);

            meshtastic_MeshPacket tempPacket = meshtastic_MeshPacket_init_default;
            
            tempPacket.id = generatePacketId();
            tempPacket.to = mp->from;
            tempPacket.from = nodeDB->getNodeNum(); // Explicitly set sender
            tempPacket.hop_limit = mp->hop_limit;   // Match the original hop limit
            
            tempPacket.which_payload_variant = meshtastic_MeshPacket_decoded_tag;
            tempPacket.decoded.portnum = meshtastic_PortNum_TEXT_MESSAGE_APP;
            tempPacket.decoded.want_response = false; // Usually false for echo to avoid ACK storms

            size_t echoLen = (size_t)written;
            tempPacket.decoded.payload.size = echoLen;
            memcpy(tempPacket.decoded.payload.bytes, echoBuf, echoLen);

            meshtastic_MeshPacket *reply = packetPool.allocCopy(tempPacket);
            if (reply) {
                // Use RX_SRC_LOCAL to tell the router this was generated by US
                this->sendToMesh(reply, RX_SRC_LOCAL, true); 
            }
        }
    }
    // --- END ECHO LOGIC ---
