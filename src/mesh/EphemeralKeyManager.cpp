...
EphemeralKeyManager::EphemeralKeyManager() : localKeyId(0), localKeyTimestamp(0), messagesSinceRotation(0), remoteKeyCount(0), initialized(false) {
...
void EphemeralKeyManager::init() {
  ...
  if (loadFromDisk()) {
    ...
    LOG_INFO("PFS: Loaded from flash - keyId=%u, age=%u hours, msgs=%u/%u, "
             "remoteKeys=%u",
             localKeyId, keyAgeHours, messagesSinceRotation, DEFAULT_EPHEMERAL_KEY_ROTATION_MESSAGES, remoteKeyCount);
...
#ifdef PFS_TEST_MODE
  LOG_WARN("PFS: TEST MODE ENABLED - rotation after %u messages", DEFAULT_EPHEMERAL_KEY_ROTATION_MESSAGES);
#endif

  LOG_INFO("PFS: Ready - keyId=%u, pubkey[0:3]=%02x%02x%02x%02x", localKeyId, localPubKey[0], localPubKey[1], localPubKey[2], localPubKey[3]);
#else
...
void EphemeralKeyManager::rotateKey() {
  ...
  LOG_INFO("PFS: Key rotated, new keyId=%u, broadcasting updated NodeInfo", localKeyId);
...
const uint8_t *EphemeralKeyManager::getPublicKey() const { return initialized ? localPubKey : nullptr; }

const uint8_t *EphemeralKeyManager::getPrivateKey() const { return initialized ? localPrivKey : nullptr; }

uint32_t EphemeralKeyManager::getKeyId() const { return localKeyId; }

uint32_t EphemeralKeyManager::getKeyTimestamp() const { return localKeyTimestamp; }

bool EphemeralKeyManager::shouldRotate() const {
  // Check message count threshold
  if (messagesSinceRotation >= DEFAULT_EPHEMERAL_KEY_ROTATION_MESSAGES) {
    LOG_DEBUG("Rotation needed: message limit reached (%d)", messagesSinceRotation);
    return true;
  }
...
void EphemeralKeyManager::setRemoteKey(uint32_t nodeNum, const uint8_t *pubkey, uint32_t keyId, uint32_t timestamp) {
  ...
  for (int i = 0; i < remoteKeyCount; i++) {
    if (remoteKeys[i].nodeNum == nodeNum) {
      ...
      if (keyId > remoteKeys[i].keyId || timestamp > remoteKeys[i].timestamp) {
        ...
        LOG_DEBUG("Updated ephemeral key for node %08x, keyId=%u", nodeNum, keyId);
      }
      return;
    }
  }
...
const RemoteEphemeralKey *EphemeralKeyManager::getRemoteKey(uint32_t nodeNum) const {
  for (int i = 0; i < remoteKeyCount; i++) {
    if (remoteKeys[i].nodeNum == nodeNum) {
      return &remoteKeys[i];
    }
...
bool EphemeralKeyManager::hasRemoteKey(uint32_t nodeNum) const { return getRemoteKey(nodeNum) != nullptr; }
...
int EphemeralKeyManager::findSlotForRemoteKey() {
  ...
  LOG_DEBUG("Evicting LRU ephemeral key for node %08x", remoteKeys[oldestSlot].nodeNum);
  return oldestSlot;
}
...
bool EphemeralKeyManager::loadFromDisk() {
  ...
  // Verify magic number
  uint32_t magic;
  if (file.read((uint8_t *)&magic, sizeof(magic)) != sizeof(magic) || magic != EPHEMERAL_KEY_MAGIC) {
    file.close();
  ...
  if (version > EPHEMERAL_KEY_VERSION) {
    LOG_WARN("PFS: File version %u newer than supported %u, regenerating", version, EPHEMERAL_KEY_VERSION);
    file.close();
    return false;
  }

  // Read local key data
  if (file.read(localPubKey, EPHEMERAL_KEY_SIZE) != EPHEMERAL_KEY_SIZE || file.read(localPrivKey, EPHEMERAL_KEY_SIZE) != EPHEMERAL_KEY_SIZE ||
      file.read((uint8_t *)&localKeyId, sizeof(localKeyId)) != sizeof(localKeyId) ||
      file.read((uint8_t *)&localKeyTimestamp, sizeof(localKeyTimestamp)) != sizeof(localKeyTimestamp) ||
      file.read((uint8_t *)&messagesSinceRotation, sizeof(messagesSinceRotation)) != sizeof(messagesSinceRotation)) {
    file.close();
  ...
  if (file.read((uint8_t *)&remoteKeyCount, sizeof(remoteKeyCount)) == sizeof(remoteKeyCount)) {
  ...
  if (hasChecksum) {
    ...
    if (file.read((uint8_t *)&storedChecksum, sizeof(storedChecksum)) == sizeof(storedChecksum)) {
      ...
      if (storedChecksum != expectedChecksum) {
        LOG_WARN("PFS: Checksum mismatch (stored=%08x, expected=%08x), regenerating", storedChecksum, expectedChecksum);
        file.close();
        return false;
      }
