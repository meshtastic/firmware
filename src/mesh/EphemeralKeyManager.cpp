...
EphemeralKeyManager::EphemeralKeyManager()
    : localKeyId(0), localKeyTimestamp(0), messagesSinceRotation(0), remoteKeyCount(0), initialized(false)
{
...
void EphemeralKeyManager::init()
{
    ...
    LOG_INFO("PFS: Ready - keyId=%u, pubkey[0:3]=%02x%02x%02x%02x", localKeyId, localPubKey[0], localPubKey[1], localPubKey[2],
             localPubKey[3]);
#else
...
uint32_t EphemeralKeyManager::getKeyId() const
{
    return localKeyId;
}
