...
static void lsIdle()
{
    ...
    const uint64_t configuredSleepTime = config.power.ls_secs * 1000ULL;
    if (sleepTime >= configuredSleepTime) {
        powerFSM.trigger(EVENT_WAKE_TIMER);
        return;
    }
    sleepLeft = configuredSleepTime - sleepTime;
...
#if HAS_ESP32_DYNAMIC_LIGHT_SLEEP
    // It's better to exit dynamic light sleep when a packet is received so routing runs promptly.
...
#ifdef ARCH_ESP32
    bool shouldLightSleep = isRouter || config.power.is_power_saving;

#if !HAS_ESP32_DYNAMIC_LIGHT_SLEEP
    const bool isTrackerOrSensor =
        IS_ONE_OF(config.device.role, meshtastic_Config_DeviceConfig_Role_TRACKER,
                  meshtastic_Config_DeviceConfig_Role_TAK_TRACKER, meshtastic_Config_DeviceConfig_Role_SENSOR);
    shouldLightSleep = shouldLightSleep && !isTrackerOrSensor;
#if HAS_WIFI && !defined(MESHTASTIC_EXCLUDE_WIFI)
    shouldLightSleep = shouldLightSleep && !isWifiAvailable();
#endif
#endif
