...
float tcxoVoltage =
    SX126X_DIO3_TCXO_VOLTAGE; // if TCXO is optional, put this here so it can be
...
meshtastic::BluetoothStatus *bluetoothStatus =
    new meshtastic::BluetoothStatus();
...
ScanI2C::FoundDevice rgb_found =
    ScanI2C::FoundDevice(ScanI2C::DeviceType::NONE, ScanI2C::ADDRESS_NONE);
...
// scan
std::pair<uint8_t, TwoWire *>
    nodeTelemetrySensorsMap[_meshtastic_TelemetrySensorType_MAX + 1] = {};
#endif
...
const char *getDeviceName() {
  ...
  if (strcmp(owner.short_name, name) != 0) {
    snprintf(name, sizeof(name), "%s_%02x%02x", owner.short_name, dmac[4],
             dmac[5]);
  } else {
 ...
 */
void printInfo() {
  LOG_INFO("S:B:%d,%s,%s,%s", HW_VENDOR, optstr(APP_VERSION), optstr(APP_ENV),
           optstr(APP_REPO));
}
...
void setup() {
...
#ifdef USE_SEGGER
  auto mode = false ? SEGGER_RTT_MODE_BLOCK_IF_FIFO_FULL
  ...
  std::string timeCommandResult =
      exec("timedatectl status | grep synchronized | grep yes -c");
  ...
  DEBUG_PORT.printf("Version %s for %s from %s\r\n", optstr(APP_VERSION),
                    optstr(APP_ENV), optstr(APP_REPO));
  DEBUG_PORT.printf(
  ...
  pinMode(config.device.button_gpio ? config.device.button_gpio : BUTTON_PIN,
          INPUT); // default to BUTTON_PIN
#ifdef BUTTON_NEED_PULLUP
  gpio_pullup_en((gpio_num_t)(config.device.button_gpio
...
#if defined(I2C_SDA1) ||                                                       \
    (defined(NRF52840_XXAA) && (WIRE_INTERFACES_COUNT == 2))
  ...
  screen_found = screenInfo.type != ScanI2C::DeviceType::NONE
                     ? screenInfo.address
  ...
  if (screen_found.port != ScanI2C::I2CPort::NO_I2C) {
    ...
    case ScanI2C::DeviceType::SCREEN_SH1106:
      screen_model = meshtastic_Config_DisplayConfig_OledType::
          meshtastic_Config_DisplayConfig_OledType_OLED_SH1106;
      break;
...
#define UPDATE_FROM_SCANNER(FIND_FN)
    ...
    rtc_found = rtc_info.type != ScanI2C::DeviceType::NONE ? rtc_info.address
                                                           : rtc_found;
    ...
    aqi_found = aqiInfo.type != ScanI2C::DeviceType::NONE
                    ? aqiInfo.address
                    : ScanI2C::ADDRESS_NONE;
...
// Two supported RGB LED currently
    ...
    accelerometer_found = acc_info.type != ScanI2C::DeviceType::NONE
                              ? acc_info.address
                              : accelerometer_found;
    LOG_DEBUG("acc_info = %i", acc_info.type);
#endif

    scannerToSensorsMap(i2cScanner, ScanI2C::DeviceType::INA260,
                        meshtastic_TelemetrySensorType_INA260);
    scannerToSensorsMap(i2cScanner, ScanI2C::DeviceType::INA226,
                        meshtastic_TelemetrySensorType_INA226);
    scannerToSensorsMap(i2cScanner, ScanI2C::DeviceType::INA219,
                        meshtastic_TelemetrySensorType_INA219);
    scannerToSensorsMap(i2cScanner, ScanI2C::DeviceType::INA3221,
    ...
    scannerToSensorsMap(i2cScanner, ScanI2C::DeviceType::QMC5883L,
    ...
    scannerToSensorsMap(i2cScanner, ScanI2C::DeviceType::ICM20948,
                        meshtastic_TelemetrySensorType_ICM20948);
    scannerToSensorsMap(i2cScanner, ScanI2C::DeviceType::MAX30102,
                        meshtastic_TelemetrySensorType_MAX30102);
    scannerToSensorsMap(i2cScanner, ScanI2C::DeviceType::SCD4X,
                        meshtastic_TelemetrySensorType_SCD4X);
    ...
    if (config.display.oled !=
        meshtastic_Config_DisplayConfig_OledType_OLED_AUTO)
      screen_model = config.display.oled;

#if defined(USE_SH1107)
    screen_model =
        meshtastic_Config_DisplayConfig_OledType_OLED_SH1107; // set dimension
                                                              // of 128x128
    ...
    screen_model =
        meshtastic_Config_DisplayConfig_OledType_OLED_SH1107; // keep dimension
                                                              // of 128x64
#endif
    ...
    ambientLightingThread =
        new AmbientLightingThread(ScanI2C::DeviceType::NONE);
  ...
  LOG_DEBUG("SPI1.begin(SCK=%d, MISO=%d, MOSI=%d, NSS=%d)", LORA_SCK, LORA_MISO,
            LORA_MOSI, LORA_CS);
  SPI1.setFrequency(4000000);
#else
  SPI.begin(LORA_SCK, LORA_MISO, LORA_MOSI, LORA_CS);
  LOG_DEBUG("SPI.begin(SCK=%d, MISO=%d, MOSI=%d, NSS=%d)", LORA_SCK, LORA_MISO,
            LORA_MOSI, LORA_CS);
  SPI.setFrequency(4000000);
...
#if defined(ST7701_CS) || defined(ST7735_CS) || defined(USE_EINK) ||           \
    defined(ILI9341_DRIVER) || defined(ILI9342_DRIVER) ||                      \
    defined(ST7789_CS) || defined(HX8357_CS) || defined(USE_ST7789) ||         \
    defined(ILI9488_CS) || defined(ST7796_CS) || defined(USE_SPISSD1306) ||    \
    defined(USE_ST7796) || defined(HACKADAY_COMMUNICATOR)
      screen =
          new graphics::Screen(screen_found, screen_model, screen_geometry);
#elif defined(ARCH_PORTDUINO)
      if ((screen_found.port != ScanI2C::I2CPort::NO_I2C ||
           portduino_config.displayPanel) &&
    ...
    LOG_DEBUG("Use compiled/slipstreamed %s",
              slipstreamTZString); // important, removing this clobbers our
                                   // magic string
    if (*config.device.tzdef && config.device.tzdef[0] != 0) {
      ...
      if (config.network.enabled_protocols &
...
// buttons are now inputBroker, so have to come after setupModules
      ...
      // Some platforms (nrf52) have a SENSE variant which allows wake from
      // sleep - override what OneButton did
      ...
      if (portduino_config.userButtonPin.enabled) {
        ...
        LOG_DEBUG("Use GPIO%02d for button",
                  portduino_config.userButtonPin.pin);
      ...
      int _pinNum = config.device.button_gpio ? config.device.button_gpio
                                              : USERPREFS_BUTTON_PIN;
#else
      int _pinNum =
...
// Do this after service.init (because that clears error_code)
      ...
      if (!pmu_found)
        RECORD_CRITICALERROR(
            meshtastic_CriticalErrorCode_NO_AXP192); // Record a hardware fault
...
// the current region name)
    ...
    defined(ILI9341_DRIVER) || defined(ILI9342_DRIVER) ||                      \
    defined(ST7789_CS) || defined(HX8357_CS) || defined(USE_ST7789) ||         \
    defined(ILI9488_CS) || defined(ST7796_CS) || defined(USE_ST7796) ||        \
    defined(USE_SPISSD1306) || defined(HACKADAY_COMMUNICATOR)
      if (screen)
        screen->setup();
#elif defined(ARCH_PORTDUINO)
      if ((screen_found.port != ScanI2C::I2CPort::NO_I2C ||
      ...
      auto loraModuleInterface = [](LockingArduinoHal *hal,
      ...
      LOG_DEBUG(
          "Activate %s radio on SPI port %s",
          portduino_config.loraModules[portduino_config.lora_module].c_str(),
      ...
      rIf = loraModuleInterface((LockingArduinoHal *)RadioLibHAL,
                                portduino_config.lora_cs_pin.pin,
                                portduino_config.lora_irq_pin.pin,
                                portduino_config.lora_reset_pin.pin,
  ...
  LockingArduinoHal *RadioLibHAL = new LockingArduinoHal(SPI, spiSettings);
      ...
      // radio init MUST BE AFTER service.init, so we have our radio config
      // settings (from nodedb init)
#if defined(USE_STM32WLx)
      if (!rIf) {
        rIf = new STM32WLE5JCInterface(RadioLibHAL, SX126X_CS, SX126X_DIO1,
                                       SX126X_RESET, SX126X_BUSY);
        if (!rIf->init()) {
      ...
      if ((!rIf) && (config.lora.region !=
                     meshtastic_Config_LoRaConfig_RegionCode_LORA_24)) {
        rIf = new RF95Interface(RadioLibHAL, LORA_CS, RF95_IRQ, RF95_RESET,
                                RF95_DIO1);
        if (!rIf->init()) {
    ...
    !defined(TCXO_OPTIONAL) && RADIOLIB_EXCLUDE_SX126X != 1
      if ((!rIf) && (config.lora.region !=
                     meshtastic_Config_LoRaConfig_RegionCode_LORA_24)) {
        auto *sxIf = new SX1262Interface(RadioLibHAL, SX126X_CS, SX126X_DIO1,
                                         SX126X_RESET, SX126X_BUSY);
      ...
      if ((!rIf) && (config.lora.region !=
        ...
        sxIf->setTCXOVoltage(SX126X_DIO3_TCXO_VOLTAGE);
        if (!sxIf->init()) {
          LOG_WARN("No SX1262 radio with TCXO, Vref %fV",
                   SX126X_DIO3_TCXO_VOLTAGE);
          delete sxIf;
          rIf = NULL;
        } else {
          LOG_INFO("SX1262 init success, TCXO, Vref %fV",
                   SX126X_DIO3_TCXO_VOLTAGE);
          rIf = sxIf;
          radioType = SX1262_RADIO;
        }
      ...
      if ((!rIf) && (config.lora.region !=
        ...
        // instead
        rIf = new SX1262Interface(RadioLibHAL, SX126X_CS, SX126X_DIO1,
                                  SX126X_RESET, SX126X_BUSY);
        if (!rIf->init()) {
        ...
        if ((!rIf) && (config.lora.region !=
                       meshtastic_Config_LoRaConfig_RegionCode_LORA_24)) {
          // try using the specified TCXO voltage
          auto *sxIf = new SX1268Interface(RadioLibHAL, SX126X_CS, SX126X_DIO1,
                                           SX126X_RESET, SX126X_BUSY);
          sxIf->setTCXOVoltage(SX126X_DIO3_TCXO_VOLTAGE);
          if (!sxIf->init()) {
            LOG_WARN("No SX1268 radio with TCXO, Vref %fV",
                     SX126X_DIO3_TCXO_VOLTAGE);
            delete sxIf;
            rIf = NULL;
          } else {
            LOG_INFO("SX1268 init success, TCXO, Vref %fV",
                     SX126X_DIO3_TCXO_VOLTAGE);
            rIf = sxIf;
        ...
        if ((!rIf) && (config.lora.region !=
                       meshtastic_Config_LoRaConfig_RegionCode_LORA_24)) {
          rIf = new SX1268Interface(RadioLibHAL, SX126X_CS, SX126X_DIO1,
                                    SX126X_RESET, SX126X_BUSY);
          if (!rIf->init()) {
        ...
        if ((!rIf) && (config.lora.region !=
                       meshtastic_Config_LoRaConfig_RegionCode_LORA_24)) {
          rIf = new LLCC68Interface(RadioLibHAL, SX126X_CS, SX126X_DIO1,
                                    SX126X_RESET, SX126X_BUSY);
          if (!rIf->init()) {
        ...
        if ((!rIf) && (config.lora.region !=
                       meshtastic_Config_LoRaConfig_RegionCode_LORA_24)) {
          rIf = new LR1110Interface(RadioLibHAL, LR1110_SPI_NSS_PIN,
                                    LR1110_IRQ_PIN, LR1110_NRESET_PIN,
                                    LR1110_BUSY_PIN);
          if (!rIf->init()) {
        ...
        if (!rIf) {
          rIf = new LR1120Interface(RadioLibHAL, LR1120_SPI_NSS_PIN,
                                    LR1120_IRQ_PIN, LR1120_NRESET_PIN,
                                    LR1120_BUSY_PIN);
          if (!rIf->init()) {
        ...
        if (!rIf) {
          rIf = new LR1121Interface(RadioLibHAL, LR1121_SPI_NSS_PIN,
                                    LR1121_IRQ_PIN, LR1121_NRESET_PIN,
                                    LR1121_BUSY_PIN);
          if (!rIf->init()) {
        ...
        if (!rIf) {
          rIf = new SX1280Interface(RadioLibHAL, SX128X_CS, SX128X_DIO1,
                                    SX128X_RESET, SX128X_BUSY);
          if (!rIf->init()) {
        ...
        lateInitVariant(); // Do board specific init (see
                           // extra_variants/README.md for documentation)
    ...
    (defined(INPUTDRIVER_ENCODER_TYPE) && INPUTDRIVER_ENCODER_TYPE == 2)
        ...
        if (!rIf)
          ...
          LOG_DEBUG("LoRA bitrate = %f bytes / sec",
                    (float(meshtastic_Constants_DATA_PAYLOAD_LEN) /
                     (float(rIf->getPacketTime(
                         meshtastic_Constants_DATA_PAYLOAD_LEN)))) *
                        1000);
        }

        // This must be _after_ service.init because we need our preferences
        // loaded from flash to have proper timeout values
        PowerFSM_setup(); // we will transition to ON in a couple of seconds,
                          // FIXME, only do this for cold boots, not waking from
                          // SDS
      ...
      uint32_t
          shutdownAtMsec; // If not zero we will shutdown at this time (used to
                          // shutdown from python or mobile client)

      // If a thread does something that might need for it to be rescheduled
      // ASAP it can set this flag This will suppress the current delay and
      // instead try to run ASAP.
      bool runASAP;

      extern meshtastic_DeviceMetadata getDeviceMetadata() {
        meshtastic_DeviceMetadata deviceMetadata;
        strncpy(deviceMetadata.firmware_version, optstr(APP_VERSION),
                sizeof(deviceMetadata.firmware_version));
        ...
        deviceMetadata.excluded_modules =
            meshtastic_ExcludedModules_EXCLUDED_NONE;
...
// graphics
    ...
    !defined(MESHTASTIC_INCLUDE_NICHE_GRAPHICS)
        deviceMetadata.excluded_modules |=
...
#if defined(ARCH_RP2040) || defined(ARCH_PORTDUINO) ||                         \
    defined(ARCH_STM32WL) || defined(CONFIG_IDF_TARGET_ESP32C6)
        deviceMetadata.excluded_modules |=
            meshtastic_ExcludedModules_BLUETOOTH_CONFIG;
#endif

#if defined(ARCH_NRF52) &&                                                     \
    !HAS_ETHERNET // nrf52 doesn't have network unless it's a RAK ethernet
                  // gateway currently
        deviceMetadata.excluded_modules |=
            meshtastic_ExcludedModules_NETWORK_CONFIG; // No network on nRF52
#elif defined(ARCH_RP2040) && !HAS_WIFI && !HAS_ETHERNET
  deviceMetadata.excluded_modules |=
      meshtastic_ExcludedModules_NETWORK_CONFIG; // No network on RP2040
#endif
...
#if !MESHTASTIC_EXCLUDE_I2C
      void scannerToSensorsMap(
          const std::unique_ptr<ScanI2CTwoWire> &i2cScanner,
        ...
        if (found.type != ScanI2C::DeviceType::NONE) {
          ...
          nodeTelemetrySensorsMap[sensorType].second =
