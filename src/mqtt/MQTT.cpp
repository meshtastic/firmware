...
inline void onReceiveProto(char *topic, byte *payload, size_t length) {
  ...
  if (!(strcmp(e.channel_id, "PKI") == 0 || (strcmp(e.channel_id, channels.getGlobalId(ch.index)) == 0 && ch.settings.downlink_enabled))) {
    return;
  }
  ...
  } else if (router && perhapsDecode(p.get()) == DecodeState::DECODE_SUCCESS) // ignore messages if we don't have the channel key
    router->enqueueReceivedMessage(p.release());
}
...
inline void onReceiveJson(byte *payload, size_t length) {
  ...
  if (json["type"]->AsString().compare("sendtext") == 0 && json["payload"]->IsString()) {
    ...
    if (json.find("channel") != json.end() && json["channel"]->IsNumber() && (json["channel"]->AsNumber() < channels.getNumChannels()))
    ...
    if (json.find("hopLimit") != json.end() && json["hopLimit"]->IsNumber())
      p->hop_limit = json["hopLimit"]->AsNumber();
    p->decoded.payload.size = pb_encode_to_bytes(p->decoded.payload.bytes, sizeof(p->decoded.payload.bytes), &meshtastic_Position_msg,
...
bool isDefaultServer(const String &host) { return host.length() == 0 || host == default_mqtt_address; }

bool isDefaultRootTopic(const String &root) { return root.length() == 0 || root == default_mqtt_root; }
...
bool connectPubSub(const PubSubConfig &config, PubSubClient &pubSub, Client &client) {
  ...
  LOG_INFO("Connecting directly to MQTT server %s, port: %d, username: %s, password: %s", config.serverAddr.c_str(), config.serverPort,
 ...
 */
bool wantsLink() {
  const bool hasChannelorMapReport = moduleConfig.mqtt.enabled && (moduleConfig.mqtt.map_reporting_enabled || channels.anyMqttEnabled());
  return hasChannelorMapReport && (moduleConfig.mqtt.proxy_to_client_enabled || isConnectedToNetwork());
}
} // namespace

void MQTT::mqttCallback(char *topic, byte *payload, unsigned int length) { mqtt->onReceive(topic, payload, length); }
...
#else
MQTT::MQTT()
    : concurrency::OSThread("mqtt"), mqttQueue(MAX_MQTT_QUEUE)
#endif
{
  if (moduleConfig.mqtt.enabled) {
    ...
    if (moduleConfig.mqtt.map_reporting_enabled && moduleConfig.mqtt.has_map_report_settings) {
      map_position_precision =
          Default::getConfiguredOrDefault(moduleConfig.mqtt.map_report_settings.position_precision, default_map_position_precision);
      map_publish_interval_msecs =
          Default::getConfiguredOrDefaultMs(moduleConfig.mqtt.map_report_settings.publish_interval_secs, default_map_publish_interval_secs);
    }
...
void MQTT::onSend(const meshtastic_MeshPacket &mp_encrypted, const meshtastic_MeshPacket &mp_decoded, ChannelIndex chIndex) {
  ...
  if (mp_decoded.which_payload_variant == meshtastic_MeshPacket_decoded_tag) {
    ...
    if (isConfiguredForDefaultServer &&
        (mp_decoded.decoded.portnum == meshtastic_PortNum_RANGE_TEST_APP || mp_decoded.decoded.portnum == meshtastic_PortNum_DETECTION_SENSOR_APP)) {
...
void MQTT::perhapsReportToMap() {
  ...
  if (map_position_precision < 12 || map_position_precision > 15) {
    LOG_WARN("MQTT Map report position precision %u is out of range, using default %u", map_position_precision, default_map_position_precision);
  ...
  mp->decoded.payload.size = pb_encode_to_bytes(mp->decoded.payload.bytes, sizeof(mp->decoded.payload.bytes), &meshtastic_MapReport_msg, &mapReport);

  // Generate node ID from nodenum for service envelope
  std::string nodeId = nodeDB->getNodeId();

  // Encode the MeshPacket into a binary ServiceEnvelope and publish
  const meshtastic_ServiceEnvelope se = {.packet = mp,
                                         .channel_id =
                                             (char *)channels.getGlobalId(channels.getPrimaryIndex()), // Use primary channel as the channel_id
                                         .gateway_id = const_cast<char *>(nodeId.c_str())};
