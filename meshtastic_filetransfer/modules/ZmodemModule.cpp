...
ProcessMessage ZmodemModule::handleDataPacket(const meshtastic_MeshPacket &mp)
{
    ...
    if (session->state != AkitaMeshZmodem::TransferState::RECEIVING) {
        LOG_DEBUG("ZmodemModule: Session %d not in RECEIVING state (state=%d), ignoring\n",
    ...
    // Feed packet to the zmodem library
    // Note: This assumes processDataPacket is adapted to work with meshtastic_MeshPacket
    // If not, we may need to create an adapter
    if (session->zmodemInstance) {
        // TODO: Verify AkitaMeshZmodem::processDataPacket signature
...
void ZmodemModule::sendReply(const String& message, NodeNum destinationNodeId, bool wantAck)
{
    ...
    packet->decoded.portnum = AKZ_ZMODEM_COMMAND_PORTNUM;
